...
void asymmetryBIN() {
 ...
 TCut Bin1="(Xcmtheta)<-0.5";
TCut Bin2="abs(Xcmtheta)<0.5";
TCut Bin3="(Xcmtheta)>0.5";
...
c1->Divide(2,0);
c1->cd(1); 
newt->Draw("cos(PIangle)>>h1",mycut&&oneB&&Bin1);
c1->cd(2);
newt->Draw("cos(PIangle)>>h2",mycut&&twoB&&Bin1);
 neg=h1->GetEntries();
 pos=h2->GetEntries();
...
c2->Divide(2,0);
c2->cd(1); 
newt->Draw("cos(PIangle)>>h3",mycut&&oneB&&Bin2);
c2->cd(2);
newt->Draw("cos(PIangle)>>h4",mycut&&twoB&&Bin2);
 neg=h3->GetEntries();
 pos=h4->GetEntries();
...
c3->Divide(2,0);
c3->cd(1); 
newt->Draw("cos(PIangle)>>h5",mycut&&oneB&&Bin3);
c3->cd(2);
newt->Draw("cos(PIangle)>>h6",mycut&&twoB&&Bin3);
 neg=h5->GetEntries();
 pos=h6->GetEntries();
