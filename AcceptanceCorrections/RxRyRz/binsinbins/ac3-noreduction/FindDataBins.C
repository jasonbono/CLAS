...
//	TTree *tMC = (TTree*)fMC->Get("v05");
    ...
    TFile *fMC = TFile::Open("/Users/Bono/clas/rootfiles/g12/2kppim/v15/hadded/2kppim-v15_Basic_Mass3.root");
    TTree *tMC = (TTree*)fMC->Get("v15");

	const int Ni = 100; //number of bins displayed in histograms
	const int Nb = 3; //number of energy bins to derive
	
	const float blow = 2.8;
	const float bhigh = 5.5;
	const int Nx = 3; //number of Xi bins to derive
	const float xlow = -1;
	const float xhigh = 1;
