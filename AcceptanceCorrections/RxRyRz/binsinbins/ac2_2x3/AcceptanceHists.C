...
void AcceptanceHists()
{
	...
	ofstream myParFile,myGetParCovMatrixFile;
	...
	cout <<Form("The hists from %s%s will be evaluated",choice.c_str(),fid.c_str()) <<endl;
	TFile* fHistos = new TFile(Form("./hists/PionHists-%s-%i_%i_%ibins%s.root",choice.c_str(),Nk,Nbq,Ncq,fid.c_str()));	
	TFile* fAC = new TFile(Form("./hists/AcceptaceHists-%s-%i_%i_%ibins%s.root",choice.c_str(),Nk,Nbq,Ncq,fid.c_str()),"RECREATE");
	myParFile.open (Form("./txt/RxRyRz-%s-%i_%i_%ibins%s.txt",choice.c_str(),Nk,Nbq,Ncq,fid.c_str()));
	myCovMatrixFile.open (Form("./txt/CovMatrixfile-%s-%i_%i_%ibins%s.txt",choice.c_str(),Nk,Nbq,Ncq,fid.c_str()));
