void FindSimBins()
{
	...
	const int Nb = 20; //number of energy bins to derive
	const float blow = 2.8;
	const float bhigh = 5.5;
	const int Nx = 20; //number of Xi bins to derive
	const float xlow = -1;
	...
	TCut bcut[Nb], bcut_display[Nb];
	for (int b=0; b<Nb; b++)
	{
		bcut_display[b] =  Form(" exibin[0][%i] = \" beam > %f && beam < %f\"; ",b,bVal[b],bVal[b+1]);
		...
		for (int x=0; x<Nx -1; x++)
		{
			for (int i=1; i<Ni; i++)
			{
				...
				if (xpop >= xCritera)
				{
					...
					cout << "xval = "<<xval[b][x]<<" b = " <<b <<"  x = "<<x<<endl;
