...
{		
	...
	TFile *fileNew = new TFile(Form("./NewRootFiles/%sAc%s-%i_%i_%ibins_%s%s.root",cchoice.c_str(),cchoiceAC.c_str(),Nk,Nbq,Ncq,cuts.c_str(),fid.c_str()),"RECREATE");
	TTree *tNew = tOriginal->CloneTree(0);
	TString tNewName = "New";
	...
	ifstream myParFile(Form("./txt/RxRyRz%s-%i_%i_%ibins_%s%s.txt",choiceAC.c_str(),Nk,Nbq,Ncq,cuts.c_str(),fid.c_str()));
	ifstream myCovMatrixFile(Form("./txt/CovMatrixfile%s-%i_%i_%ibins_%s%s.txt",choiceAC.c_str(),Nk,Nbq,Ncq,cuts.c_str(),fid.c_str()));
	if( !myParFile || !myCovMatrixFile) 
	{
		cout << "Cannot open one of the text files. \n";
		gROOT->ProcessLine(".q");
