...
Double_t fitFunction(Double_t *x, Double_t *par) {
  return background(x,par) + GaussianPeak(x,&par[5]);
}
  ...
  Double_t & myBackground, Double_t & myYield, Double_t & myRms, Double_t & myMean, Int_t & myYieldErr)
{
  ...
  fitFcn->SetParameter(3, term3);  //jb 
  fitFcn->SetParameter(4, term4);  //jb
  if(term1==0&&term2==0){
...
//fitFcn->SetParameter(3, height); // hight
  ...
  fitFcn->SetParameter(4, mean);
  fitFcn->SetParameter(5, width);  
  fitFcn->SetParLimits(5, widthLow, widthHigh);   //jb set range for the width
  ...
  histo->Draw("E");
  Double_t par[8];  //jb changed from 6 to 8
  ...
  // writes the fit results into the par array
   ...
   signalFcn->SetParameters(&par[5]); //jb changed from 3 to 5
   ...
   Double_t Intg = signalFcn->Integral(par[6]-factor*par[7],par[6]+factor*par[7]); //jb moved indecies up by 2
   Double_t Intb = backFcn->Integral(par[6]-factor*par[7],par[6]+factor*par[7]);  //jb moved indecies up by 2
  // cout<<"Integral of function: (intg) "<<Intg<<" "<<" and (intb) "<<Intb<<endl;
   ...
   Int_t N1Err=yield*sqrt(((fitFcn->GetParError(5))/par[5])**2+((fitFcn->GetParError(7))/par[7])**2); //jb moved indecies up by 2
...
//now i want to parse the number yieldb
  ...
  Double_t starty3=0.85*histo->GetMaximum();
   ...
   TLatex *sum2=new TLatex(startx, starty2,Form("M:%5.4f #pm %5.4f",par[6], M1Err));//jb moved indecies up by 2
   TLatex *sum3=new TLatex(startx, starty3,Form("#sigma:%5.4f #pm %5.4f",par[7], G1Err));//jb moved indecies up by 2
   sum->SetTextSize(0.05);
   sum->SetTextColor(4);  //jb X  this is the N: color
   sum->Draw("same");
	sum1->SetTextSize(0.05); //jb jan11 2012 
  ...
  myBackground = yieldb;
  myYield = yield;
  myRms = par[7]; //jb moved indecies up by 2
  myMean = par[4]; //jb moved indecies up by 2
  myYieldErr = N1Err;
  return yield;
 }
...
void fitBW(TH1F *histo, Int_t tryno, Double_t low, Double_t high, Double_t term0, Double_t term1,Double_t term2, Double_t mean, Double_t width, Double_t factor)
{
  ...
  Double_t par[8]; //jb moved index from 6 to 8
