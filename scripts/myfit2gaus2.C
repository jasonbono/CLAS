...
jan 13 changed the function fitGAUS to fitGAUS2  to represent the name of the script and the number of variables that are passed.
...
Double_t  fitGAUS2(TH1F *histo, Int_t tryno, Double_t low, Double_t high, Double_t term0, 
  Double_t term1,Double_t term2, Double_t mean, Double_t width, Double_t factor, 
  Double_t & myBackground, Double_t & myYield)
{
  //TText *sum[100];
...
//fitFcn->SetParameter(3, height); // hight
  ...
  // writes the fit results into the par array
   ...
   Int_t N1Err=yield*sqrt(((fitFcn->GetParError(3))/par[3])**2+((fitFcn->GetParError(5))/par[5])**2);
...
//now i want to parse the number yieldb
  ...
  //TLatex *sum=new TLatex(startx, starty,Form("Ye_t starty0=0.85*histo->GetMaximum();
   ...
   TLatex *sum2=new TLatex(startx, starty2,Form("M:%5.4f #pm %5.4f",par[4], M1Err));
   TLatex *sum3=new TLatex(startx, starty3,Form("#sigma:%5.4f #pm %5.4f",par[5], G1Err));
