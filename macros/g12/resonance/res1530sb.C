...
//====================================================================
 ...
 TH1F *h0pk=new TH1F("h0pk","",35,1.15,1.5);
 TH1F *h0sb=new TH1F("h0sb","",35,1.15,1.5);
...
// this is for normalisation
 ...
 //Float_t nbg=yieldb
 Double_t nsb = h0sb->GetEntries();
 // nplot=nbg   nplot=nsb*fac=nbg -> fac=nbg/nsb so multiply the hist by fac
 Double_t fac = myBackground/nsb;
 cout <<"the number of events in sidebands is: " << nsb << endl;
cout <<"the number of events in background is: " << myBackground << endl;
 cout <<"the normalisation factor is: (myBackground/nsb)= " << fac <<endl;
 //*/
...
/* taking this out
 ...
 h1sb->GetXaxis()->SetTitle("M(#pi^{-}#Xi^{0}) (GeV/c^{2})");
 ...
 h1sbNorm->Add(h1sb,fac);
 h1sbNorm->SetFillColor(2);
 h1sbNorm->SetFillStyle(3004);
 h1sbNorm->Draw("same");
 

 //now adding the plot of the subtracted hists
 TCanvas *c1b=new TCanvas("c1b","",1200,800);
 TH1F *h1Sub=new TH1F("h1Sub","",60,1.4,2);
 h1Sub->Add(h1,1);
 h1Sub->Add(h1sbNorm,-1);
 h1Sub->GetYaxis()->SetTitle("Events/10MeV/c^{2}");  
 h1Sub->GetXaxis()->SetTitle("Sideband Subtracted M(#pi^{-}#Xi^{0}) (GeV/c^{2})");  
 h1Sub->Draw();
...
/*
 ...
 TH1F *h2sbNorm=new TH1F("h2sbNorm","",60,1.4,2);
 h2sbNorm->Add(h2sb,fac);
 h2sbNorm->SetFillColor(2);
 h2sbNorm->SetFillStyle(3004);
 h2sbNorm->Draw("same");


 //now adding the plot of the subtracted hists
 TCanvas *c2b=new TCanvas("c2b","",1200,800);
 TH1F *h2Sub=new TH1F("h2Sub","",60,1.4,2);
 h2Sub->Add(h2,1);
 h2Sub->Add(h2sbNorm,-1);
 h2Sub->GetYaxis()->SetTitle("Events/10MeV/c^{2}");  
 h2Sub->GetXaxis()->SetTitle("Sideband Subtracted MM(K^{+}K^{+}) (GeV/c^{2})");  
 h2Sub->Draw();


cout << "the number of entries in the original plots are: " << h1->GetEntries() <<" and: " << h2->GetEntries() << endl;
cout << "the integral the original plots are: " << h1->Integral() <<" and: " << h2->Integral() << endl;

cout << "the number of entries in the sb plots are: " << h1sb->GetEntries() <<" and: " << h2sb->GetEntries() << endl;
cout << "the integral of the sb plots are: " << h1sb->Integral() <<" and: " << h2sb->Integral() << endl;

cout << "the number of entries in the normalized sb plots are: " << h1sbNorm->GetEntries() <<" and: " << h2sbNorm->GetEntries() << endl;
cout << "the integral of the normalized sb plots are: " << h1sbNorm->Integral() <<" and: " << h2sbNorm->Integral() << endl;

cout << "the number of events in the subtracted hists are (false reading because of negitive hists): " << h1Sub->GetEntries() << " and: " << h2Sub->GetEntries() << endl;
cout << "the integral the subtracted hists are: " << h1Sub->Integral() << " and: " << h2Sub->Integral() << endl;

//*/


/*
	c0->Print("~/clas/pics/12_jan16/g12talk/ressideband/mmkpkppimFit.eps");
	c00->Print("~/clas/pics/12_jan16/g12talk/ressideband/mmkpkppimSideband.eps");
	c1->Print("~/clas/pics/12_jan16/g12talk/ressideband/mpimXiSideband.eps");
	c1b->Print("~/clas/pics/12_jan16/g12talk/ressideband/mpimXiSubtraction.eps");
	c2->Print("~/clas/pics/12_jan16/g12talk/ressideband/mmkpkpSideband.eps");
	c2b->Print("~/clas/pics/12_jan16/g12talk/ressideband/mmkpkpSubtraction.eps");
*/	
