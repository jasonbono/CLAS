// this is somthing that lei wrote for g11 data.  i will use it as a guide for g12 making comments on what cuts changed



TCut sc="abs(sctkp1-sclkp1/(betakp1*29.99)-scv)<0.5"  // similar
  TCut sc2="abs(sctkp2-sclkp2/(betakp2*29.99)-scv)<1"  //simalar
  TCut ks="abs(mkp1pim-0.892)<0.08"//  ask about the value
  TCut ks2="abs(mkp2pim-0.892)<0.08" //ask about the value
  TCut st="abs(scv-stv)<1"      // same
  TCut vcut="abs(vz+10)<21&&(vx^2+vy^2)<8"  // this is changed to 90 cm down, make sure its still a 2.8cm sided target
  TCut vout="(vx^2+vy^2)>8"  //why cut it out?

  TCut cas="abs(sqrt(mm2kpkp)-1.322)<3*0.0077" // similar
  TCut lambda="abs(sqrt(mm2kpkppim)-1.1162)<3*0.0088" //similar
  ...
  TCut cas2="abs(sqrt(mm2kpkppim)-1.3172)<3*0.0066"  // what is this?
  TCut cas2p="abs(sqrt(mm2kpkppim)-1.315)<3*(1+(beam-2.6)/2)*0.0074"  //what is this?
  TCut cas2pside="abs(sqrt(mm2kpkppim)-1.315)<4*(1+(beam-2.6)/4)*0.0074&&abs(sqrt(mm2kpkppim)-1.315)>4*(1+(beam-2.6)/4)*0.0074" // what is this?
  TCut cas2side="abs(sqrt(mm2kpkppim)-1.315)<6*0.0074&&abs(sqrt(mm2kpkppim)-1.315)>3*0.0074" //what is this?
