...
purpose: this will take an unaltered tree (strait from the analyzer) and will make a seperate root file with the RESONATE cut.
...
void ResonanceTree(){
	...
	//TFile *newfile = TFile::Open("/Users/Bono/clas/rootfiles/g12/resonate/v32k_small_resonate.root","RECREATE");
	...
	TFile *newfile = TFile::Open("/volatile/clas/clasg12/bono/rootfiles/v37k/hadded/v37k_full_resonate.root", "RECREATE");
	TTree *oldtree = (TTree*)oldfile->Get("v37k");
	TTree *newtree = oldtree->CloneTree(0);
	
	
	//specify the new names of the new trees;
	TString treename1 = "v37k_resonate";
