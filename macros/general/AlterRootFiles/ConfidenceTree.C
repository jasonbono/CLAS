...
void NewbasicTree(){
	...
	TFile *newfile = TFile::Open("/volatile/clas/clasg12/bono/rootfiles/v37k/hadded/v37k_full_conf.root", "RECREATE");
	TTree *oldtree = (TTree*)oldfile->Get("v37k");
	TTree *newtree = oldtree->CloneTree(0);
	
	
	//specify the new names of the new trees;
	TString newtreename = "v37k_conf";
	...
	oldtree->SetBranchAddress("kf_prob", &kf_prob);
