...
int main(int argc,char **argv)
{
  ...
  for (int iarg = optind; iarg < argc; ++iarg) {
	...
	double xi_tau = 0.1639; //in nanno second
	double xi_decaytime = expodecay(xi_tau);	
	//cout << "GAMMA = " <<xi_gamma << "  xi_decaytime = " <<xi_decaytime<<endl;
	//cout << "x-detachment = " <<lightspeed*xi_gamma*xi_px*xi_decaytime/xi_t<<endl;
	...
	//set the new vertex for pion which comes from the Xi decay
	PART->part[2].vert.x = pi_newvertx;
	PART->part[2].vert.y = pi_newverty;
	PART->part[2].vert.z = pi_newvertz;
	//give the proton and other pion the same vertex as the first pion	
	PART->part[1].vert.x = pi_newvertx;
	PART->part[1].vert.y = pi_newverty;
	PART->part[1].vert.z = pi_newvertz;	
	PART->part[0].vert.x = pi_newvertx;
	PART->part[0].vert.y = pi_newverty;
	PART->part[0].vert.z = pi_newvertz;
