...
int main(int argc,char **argv)
{
  ...
  for (int iarg = optind; iarg < argc; ++iarg) {
	...
	for (i=0;i<num_particles;i++) //jb second loop over particles
	{
		if(PART->part[i].pid== kaonID){
			if(nkaon>0){
				kp2_px = PART->part[i].p.space.x;
				kp2_py = PART->part[i].p.space.y;
				kp2_pz = PART->part[i].p.space.z;
			}else{
				nkaon++;
				kp1_px = PART->part[i].p.space.x;
				kp1_py = PART->part[i].p.space.y;
				kp1_pz = PART->part[i].p.space.z;
			}
		}
		if(PART->part[i].pid == pionID){
			pim_px = PART->part[i].p.space.x;
			pim_py = PART->part[i].p.space.y;
			pim_pz = PART->part[i].p.space.z;
			pionPosition = i;
		}
		event_vertx =	PART->part[i].vert.x;
		event_verty =	PART->part[i].vert.y;
		event_vertz =	PART->part[i].vert.z;
	}
	
	
	//jb xi information needed for calculation
	double xi_tau = 1.639e-10;
	...
	double xi_flightx = lightspeed*xi_px* xi_decaytime/xi_mass;
	double xi_flighty = lightspeed*xi_py*xi_decaytime/xi_mass;
	double xi_flightz = lightspeed*xi_pz*xi_decaytime/xi_mass;
	//find the new pion vertex
	...
	//set the new vertex for pions	
	PART->part[pionPosition].vert.x = pi_newvertx;
	PART->part[pionPosition].vert.y = pi_newverty;
	PART->part[pionPosition].vert.z = pi_newvertz;
