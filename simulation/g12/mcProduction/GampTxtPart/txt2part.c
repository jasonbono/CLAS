...
Edited by Jason Bono july 23 2013. (see //jb comments)
acquired at https://jlabsvn.jlab.org/svnroot/clas/trunk/io/gamp2txt/
* -=======================================================- */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc,char *argv[]){
  ...
  while(fscanf(stdin, "%i", &NPart) == 1){
	...
	head[0].nrun = RunNo;
	//head[0].type = -4; //jb
	head[0].type = 1; //jb i changed this 1=physics data (as is the value in gamp2part), 4=unknown
	head[0].time = secs;
	head[0].evtclass = 15; //jb this is in gamp2part
	head[0].trigbits = 1; //jb this is in gamp2part (exept it is a different value if the -t option is chosen in gamp2part)
