void kstaryield(Double_t eg_min, Double_t eg_max, Double_t egstep){
  ...
  xxx->Draw(">>yieldlist","abs(scv-stv)<1&&abs(mppim-1.1159)<0.0048&&abs(sctkp-sclkp/(betakp*29.99)-scv)<1&&abs(mm2kpppi-0.0166)<0.0246&&nkm<1&&npip<1&&np<2&&npim<2&&nkp<2&&ngamma<3");  
  ...
  ifstream bgdat;
  char kstaryieldout[30], psfile[30], bgin[30];
  sprintf(kstaryieldout,"kstaryield_%5.4f_%5.4f.newdat",eg_min, eg_max);
  sprintf(psfile,"kstaryield_%5.4f_%5.4f.ps",eg_min, eg_max);
  sprintf(bgin,"bgterm.dat");
  bgdat.open(bgin, ios::in);
  Int_t m, p;
  m=0;
  Double_t bg0[20][10],bg1[20][10],bg2[20][10],bg3[20][10], bgbeam[20];
  while(bgdat){
    bgdat>>bgbeam[m];
    for(p=0;p<10;p++){
      bgdat>>bg0[m][p]>>bg1[m][p]>>bg2[m][p]>>bg3[m][p];
      }
    m++;
  }
  ...
  for( Int_t i=1;i<=egbin;i++){
    ...
    ps->NewPage();
	 ...
	 h[i][j]=new TH1F(Form("h%d_%d",i,j),Form("E_{#gamma}:%5.4f to %5.4f, Cos*(#theta_{K^{*+}}):%3.2f to %3.2f",elow, ehigh, tlow,thigh),40,0.6,1.5);
       ...
       //xxx->Draw(Form("conmkppi0>>h%d_%d",i,j), Form("abs(scv-stv)<1&&abs(mppim-1.1159)<0.0048&&abs(sctkp-sclkp/(betakp*29.99)-scv)<1&&abs(mm2kpppi-0.0166)<0.0246&&nkm<1&&npip<1&&np<2&&npim<2&&nkp<2&&ngamma<3&&beam<%f&&beam>%f&&(-lamcmtheta)<%f&&(-lamcmtheta)>%f",ehigh, elow, thigh, tlow));
       xxx->Draw(Form("conmkppi0>>h%d_%d",i,j), Form("beam<%f&&beam>%f&&(-lamcmtheta)<%f&&(-lamcmtheta)>%f",ehigh, elow, thigh, tlow), "E");
       yield=fitBW(h[i][j],i*10+j-1,0.65,1.2,bg0[ii][j],bg1[ii][j],bg2[ii][j],bg3[ii][j],0.892,0.051,0.7,1.15,yielderr);
       Int_t hentry=0;
...
Double_t  fitBW(TH1F *histo, Int_t try, Double_t low, Double_t high, Double_t term0, Double_t term1,Double_t term2, Double_t term3, Double_t mean, Double_t width, Double_t fitlow, Double_t fithigh, Int_t &yielderr)
{
 ...
 if(term0!=0) {fitFcn->SetParameter(0, term0);
  ...
  if(term1!=0) {fitFcn->FixParameter(1, term1);} // pol c0
  else {fitFcn->FixParameter(1,0);}
   if(term2!=0) {fitFcn->FixParameter(2, term2); }// pol c2
  else {fitFcn->FixParameter(2,0);}
  if(term3!=0) {fitFcn->FixParameter(3, term3);} // pol c3
...
//fitFcn->SetParameter(4, height); // hight
  ...
  fitFcn->SetParLimits(6, width-0.004,width+0.011);  
   fitFcn->SetLineWidth(1);  
  histo->Fit("fitFcn","EV","",low,high);
  histo->Draw("E");
  // improve the picture:
  ...
  // writes the fit results into the par array
   ...
   Double_t HeightErr=fitFcn->GetParError(4);
   if(par[4]>0){
     yielderr=yield*HeightErr/par[4];
   }
   else {
     yielderr=sqrt(yield);
   }
   //cout<<"\ngammaerr "<<GammaErr<<" "<<par[6]<<" "<<yielderr<<endl;
   cout<<"\nnormerr "<<HeightErr<<" "<<par[4]<<" "<<yielderr<<endl;
   Double_t ratio = Intg/Intb;
