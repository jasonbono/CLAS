...
#-------------------------------------------open and change GENERATE template ----------------------------------------------------------------
 	...
 	open(GENERATESCRIPT, ">$dir/$newdir/generatescript-${ebin}-${jobversion}.sh") || die "Can't open farmscript_${ebin} $!\n";
	...
	close (GENERATESCRIPT);
 	system ("chmod u+x $dir/$newdir/generatescript-${ebin}-${jobversion}.sh"); 
...
#----------------------------------------------open and change GENR8INPUT template--------------------------------------------------------
 	...
 	open(GENR8INPUTTEMPLATE, "$dir/$inputfile")|| die "Can't open genr8input_template$!\n"; 
 	open(GENR8INPUTSCRIPT, ">$dir/$newdir/genr8input-${ebin}-${jobversion}") || die "Can't open  farmscript_$ebin.v${jobversion}$!\n";
...
#-----------------------------------------open and change JOB template-----------------------------------------------
 	...
 	open (JOBSCRIPT, "> $dir/$newdir/job-script-${ebin}-${jobversion}") || die "Can't open farmjob_${ebin}$!`\n";
...
#-----------------------------------DO JSUB, INCREMENT L, AND TERMINATE THE LOOP-----------------------------------------
 ...
 #if($sys) {system ("jsub $dir/job-script-${ebin}-${jobversion}");}	#when run localy the jsub command will cause an error
 $L=$U;
}
