...
#-------------------------------------------open and change GENERATE template ----------------------------------------------------------------
 	...
 	open(GENERATESCRIPT, ">$dir/$newdir/generatescript-${ebin}.${jobversion}") || die "Can't open farmscript_${ebin} $!\n";
	...
	close (GENERATESCRIPT);
 	system ("chmod u+x $dir/$newdir/generatescript-${ebin}.${jobversion}"); 
...
#----------------------------------------------open and change GENR8INPUT template--------------------------------------------------------
 	...
 	open(GENR8INPUTSCRIPT, ">$dir/$newdir/genr8input-${ebin}.${jobversion}") || die "Can't open genr8input-$ebin.${jobversion}$!\n";
...
#-----------------------------------------open and change JOB template-----------------------------------------------
 	...
 	open (JOBSCRIPT, "> $dir/$newdir/job-script-${ebin}.${jobversion}") || die "Can't open farmjob.${ebin}$!`\n";
...
#-----------------------------------DO JSUB, INCREMENT L, AND TERMINATE THE LOOP-----------------------------------------
 ...
 if($sys) {system ("jsub $dir/$newdir/job-script-${ebin}.${jobversion}");}	#when run localy the jsub command will report error
 $L=$U;
}
