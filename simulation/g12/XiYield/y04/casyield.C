...
void casyield(Double_t eg_min, Double_t eg_max, Double_t egstep)
{
	...
	sprintf(casyieldout,"./results/casyield.dat",eg_min, eg_max);  //output data file
  	sprintf(psfile,"./results/casyield.ps",eg_min, eg_max);  //output ps file
 	...
 	for( Int_t i=1;i<=egbin;i++){
		...
		jmax=tmax/tstep;
     	for( Int_t j=0;j<jmax;j++){
			...
			//JB_y02 xxx->Draw(Form("mmkpkp>>h%d_%d",i,j), Form("kf_prob>0.01&&abs(scv-stv)<1&&beam<%f&&beam>%f&&tpkp1<%f&&tpkp1>%f",ehigh, elow, thigh, tlow));
