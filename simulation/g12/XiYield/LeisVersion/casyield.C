void casyield(Double_t eg_min, Double_t eg_max, Double_t egstep){
  ...
  char casyieldout[30], psfile[30];
   sprintf(casyieldout,"casyield_%5.4f_%5.4f.newdat",eg_min, eg_max);
  sprintf(psfile,"casyield_%5.4f_%5.4f.ps",eg_min, eg_max);
 ...
 for( Int_t i=1;i<=egbin+1;i++){
    ...
    ps->NewPage();
	 ...
	 h[i][j]=new TH1F(Form("h%d_%d",i,j),Form("E_{#gamma}:%5.4f to %5.4f, t':%3.2f to %3.2f",elow, ehigh, tlow,thigh),150,1.2,1.8);
       ...
       xxx->Draw(Form("mmkpkp>>h%d_%d",i,j), Form("abs(scv-stv)<1&&beam<%f&&beam>%f&&tpkp1<%f&&tpkp1>%f",ehigh, elow, thigh, tlow));
     ...
     casyielddat<<emean<<" ";
     for(j=0;j<=10;j++){casyielddat<<tyield[j]<<" "<<tyielderr[j]<<" ";}
