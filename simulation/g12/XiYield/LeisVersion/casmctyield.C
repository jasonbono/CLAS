void casyield(Double_t eg_min, Double_t eg_max, Double_t egstep){
  TFile *f = new TFile("newmc3.root");
  TNtuple *newmc3=(TNtuple *)f->Get("newmc3");
  //Load fit2BW.C;
  ofstream casmctyielddat;
  char casmctyieldout[30], psfile[30];
   sprintf(casmctyieldout,"casmctyield.dat");
  sprintf(psfile,"casmctyield_%4.3f_%4.3f.ps",eg_min, eg_max);
  TPostScript *ps = new TPostScript(psfile, 111);
  casmctyielddat.open(casmctyieldout, ios::out);
...
//        }
     ...
     casmctyielddat<<emean<<" ";
     for(j=0;j<=10;j++){casmctyielddat<<tyield[j]<<" "<<tyielderr[j]<<" ";}
     
     casmctyielddat<<endl;
     c2->Update();
     }
 casmctyielddat.close();
 ps->Close();
