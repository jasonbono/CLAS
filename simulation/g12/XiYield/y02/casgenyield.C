...
void casgenyield(Double_t eg_min, Double_t eg_max, Double_t egstep){
  	TFile *f = new TFile("./rootfilelinks/MC_v07.gamp.root");
  	...
  	sprintf(psfile,"./results/casgenyield.ps",eg_min, eg_max);
  	//sprintf(casgenyieldout,"./results/casgenyield_%4.3f_%4.3f.dat",eg_min, eg_max);
  	//sprintf(psfile,"./results/casgenyield_%4.3f_%4.3f.ps",eg_min, eg_max);
  	...
  	Int_t j,jmax;
  	for(j=0;j<=19;j++){ //jb changed to 19 from 9
    	tyield[j]=-1;
    	tyielderr[j]=-10;
	}
  	yielderr=0;
 	...
 	for( Int_t i=1;i<=egbin;i++){
    	...
    	if(ehigh<=2.9) {
     	...
     	for(j=0;j<=19;j++){casgenyielddat<<tyield[j]<<" "<<tyielderr[j]<<" ";} //jb changed from 9 to 19
