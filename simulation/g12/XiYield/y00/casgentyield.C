void casyield(Double_t eg_min, Double_t eg_max, Double_t egstep){
  TFile *f = new TFile("newgamp3.root");
  TNtuple *newgamp3=(TNtuple *)f->Get("newgamp3");
  //Load fit2BW.C;
  ofstream casgentyielddat;
  char casgentyieldout[30], psfile[30];
   sprintf(casgentyieldout,"casgentyield.dat");
  sprintf(psfile,"casgentyield_%4.3f_%4.3f.ps",eg_min, eg_max);
  TPostScript *ps = new TPostScript(psfile, 111);
  casgentyielddat.open(casgentyieldout, ios::out);
 ...
 for( Int_t i=1;i<=egbin;i++){
    ...
    if(ehigh<=2.9) {
	 ...
	 h[i][j]=new TH1F(Form("h%d_%d",i,j),Form("E_{#gamma}:%4.3f to %4.3f, t':%3.2f to %3.2f",elow, ehigh, tlow,thigh),100,1.2,1.8);
       ...
       newgamp3->Draw(Form("mmkpkp>>h%d_%d",i,j), Form("beam<%f&&beam>%f&&tp<%f&&tp>%f",ehigh, elow, thigh, tlow));
...
//        }
     ...
     casgentyielddat<<emean<<" ";
     for(j=0;j<=10;j++){casgentyielddat<<tyield[j]<<" "<<tyielderr[j]<<" ";}
     
     casgentyielddat<<endl;
     c2->Update();
     }
 casgentyielddat.close();
 ps->Close();
