/*
jb august 29: this reads a data kpkp ntuple and makes yeilds 

Input:
1) .root file

Output: 
1) casyield_elow_ehigh.newdat
2) casyield_elow_ehigh.ps
...
{
	TFile *f = new TFile("v37k_full_newbasic.root");
 	TNtuple *xxx=(TNtuple *)f->Get("v37k");
  	//Load fit2BW.C;
  	ofstream casyielddat;
	char casyieldout[512];  //added this to be 512 since 30 was causing segfaults 
	char psfile[512];
	
   	sprintf(casyieldout,"./results/casyield_%5.4f_%5.4f.newdat",eg_min, eg_max);
  	sprintf(psfile,"./results/casyield_%5.4f_%5.4f.ps",eg_min, eg_max);
  	...
  	Int_t yield;
  	Int_t yielderr;
  	Int_t tyield[20]; //jb changed from 10 to 20
  	Int_t tyielderr[20]; //jb changed from 10 to 20
  	Int_t j,jmax;
  	for(j=0;j<=19;j++){   //changed from 9 to 19
    tyield[j]=-1;
    tyielderr[j]=-10;}
 	...
 	for( Int_t i=1;i<=egbin+1;i++){
		...
		jmax=tmax/tstep;
		//tmax = 1;           //jb added this in for kp1cmthena inplace of tkp1
		//tstep = 0.4;        //jb added this in for kp1cmthena inplace of tkp1
    	//jmax=2*tmax/tstep;  //jb added in the factor of two for looking at kp1cmtheta inplace of tkp1
     	...
     	for( Int_t j=0;j<jmax;j++){
       		...
       		//xxx->Draw(Form("mmkpkp>>h%d_%d",i,j), Form("kf_prob>0.1&&beam<%f&&beam>%f&&tkp1<%f&&tkp1>%f",ehigh, elow, thigh, tlow));			
			xxx->Draw(Form("mmkpkp>>h%d_%d",i,j), Form("kf_prob>0.01&&beam<%f&&beam>%f&&tpkp1<%f&&tpkp1>%f",ehigh, elow, thigh, tlow));
			...
			//h[i]->GetXaxis()->SetTitleSize(0.04);
       		...
       		if(ehigh<=1.8){
	 			yield=fitGAUS(h[i][j], i*10+j-1, 1.2,1.4, 1,1,1,1.32,0.006,3,yielderr);
       		}
       		else if(ehigh<=2){
	 			yield=fitGAUS(h[i][j], i*10+j-1, 1.2, 1.4, 1,1,1,1.32,0.006,3,yielderr);
       		}
       		else if(elow>=3.6){
	 			yield=fitGAUS(h[i][j], i*10+j-1, 1.2, 1.4, 1,1,0,1.32,0.006,3,yielderr);
     	...
     	casyielddat<<emean<<" ";
     	for(j=0;j<=19;j++){casyielddat<<tyield[j]<<" "<<tyielderr[j]<<" ";}  //jb changed from 9 to 19
