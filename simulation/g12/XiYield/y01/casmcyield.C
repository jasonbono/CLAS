...
void casmcyield(Double_t eg_min, Double_t eg_max, Double_t egstep){
	TFile *f = new TFile("./rootfilelinks/MC_v07_v37k.root");
  	TNtuple *newmc3=(TNtuple *)f->Get("v37k");
  	//Load fit2BW.C;
  	ofstream casmcyielddat;
	char casmcyieldout[512];
	char psfile[512];
	sprintf(casmcyieldout,"./results/casmcyield.dat",eg_min, eg_max);
  	sprintf(psfile,"./results/casmcyield.ps",eg_min, eg_max);
   	//sprintf(casmcyieldout,"./results/casmcyield_%4.3f_%4.3f.dat",eg_min, eg_max);
  	//sprintf(psfile,"./results/casmcyield_%4.3f_%4.3f.ps",eg_min, eg_max);
 	...
 	for( Int_t i=1;i<=egbin;i++){
		...
		casmcyielddat<<emean<<" ";
    	for(j=0;j<=19;j++){casmcyielddat<<tyield[j]<<" "<<tyielderr[j]<<" ";} //jb changed from 9 to 19
