...
{
	TFile *f = new TFile("./rootfilelinks/v37k_00_94.root");
	...
	sprintf(casyieldout,"./results/casyield.dat",eg_min, eg_max);
  	sprintf(psfile,"./results/casyield.ps",eg_min, eg_max);
	...
	Int_t pads=egbin/4+1;
  	...
  	Int_t yield;
  	Int_t yielderr;
  	Int_t tyield[20];  //jb changed to 20 from 10
  	Int_t tyielderr[20]; //jb same
  	Int_t j,jmax;
  	for(j=0;j<=19;j++){ //jb changed from 9 to 19
    	tyield[j]=-1;
    	tyielderr[j]=-10;
	}
  	yielderr=0;
