...
void tslope(Int_t bins, Int_t minmc){
	//JB void getcros(Int_t bins, Int_t minmc){
	...
	//   Int_t Style[4]={8,8,8,8};
  	...
  	//JB sprintf(fluxin, "./results/casflux.dat");  
  	...
  	while(fluxdat)
	{
 	...
 	fluxdat.close();
	*/
 	sprintf(psfile, "./results/acc.eps");
   ...
   //------------------- calculate the cross sections ---------------------------------
    for(i=0;i<bins;i++)
	{
      	...
      	for(tbin=0;tbin<tbinmax[i];tbin++)
		{
			...
			if(genyield[i][tbin]>0&&mcyield[i][tbin]>minmc)
			{
	  			...
	  			//JB cros[i][tbin]=normyield[i][tbin]*0.58*1.06*1000000000*1.05*bg[i]/(BR*flux[i]*1.08);
	  			//JB croserr[i][tbin]=normyielderr[i][tbin]*0.58*1.06*1000000000*1.05*bg[i]/(BR*flux[i]*1.08);
	  	...
	  	//JB totalcros[i]=totalnormyield[i]*0.58*1.06*1000000000*1.05*bg[i]/(BR*flux[i]*1.08);
      	//totalcroserr[i]=totalcroserr[i]*0.58*1.06*1000000000/(BR*flux[i])*tstep;
      	//JB totalcroserr[i]=totalnormyielderr[i]*0.58*1.06*1000000000*1.05*bg[i]/(BR*flux[i]*1.08);
    ...
    ps->NewPage();
    c3->Clear();
    //JB TGraph *gr0=new TGraph(bins,fluxbeam,flux);
