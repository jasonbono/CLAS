...
void casyield(Double_t eg_min, Double_t eg_max, Double_t egstep)
{
	...
	TFile *f = new TFile("~/clas/rootfiles/LINKS/2kppim-v01.root");
 	TNtuple *xxx=(TNtuple *)f->Get("v01_Basic");
 	...
 	for( Int_t i=1;i<=egbin;i++){
		...
		jmax=tmax/tstep;
     	for( Int_t j=0;j<jmax;j++){
			...
			//xxx->Draw(Form("mmkpkp>>h%d_%d",i,j), Form("kf_prob>0.01&&abs(scv-stv)<1&&beam<%f&&beam>%f&&tpkp1<%f&&tpkp1>%f",ehigh, elow, thigh, tlow));
			//xxx->Draw(Form("mmkpkp>>h%d_%d",i,j), Form("nphoton_nb1<2&&abs(mmkpkppim-1.116)<0.033&&abs(vtime-(sctkp1-sclkp1/(betakp1*29.99)))<0.4&&abs(vtime-(sctkp2-sclkp2/(betakp2*29.99)))<1.1&&beam<%f&&beam>%f&&tpkp1<%f&&tpkp1>%f",ehigh, elow, thigh, tlow));
			xxx->Draw(Form("mmkpkp>>h%d_%d",i,j), Form("abs(mmkpkppim-1.116)<0.033&&beam<%f&&beam>%f&&tpkp1<%f&&tpkp1>%f",ehigh, elow, thigh, tlow)); //JB y05
			
			/* JB the cuts to use for kpkppim skim:
				abs(mmkpkppim-1.116)<0.033
				abs(vtime-(sctkp1-sclkp1/(betakp1*29.99)))<0.4
				abs(vtime-(sctkp2-sclkp2/(betakp2*29.99)))<1.1
				nphoton_nb1<2
			*/
