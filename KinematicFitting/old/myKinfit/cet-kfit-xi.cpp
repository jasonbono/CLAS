...
int main(int argc, char** argv)
{
    ...
    charged_particle cp_kp0, cp_kp1, cp_km0;
    lorentz_vector kp0, kp1, km0;
    lorentz_vector mkpkp, mkpkpkm;
    lorentz_vector kfkp0, kfkp1, kfkm0;
    lorentz_vector kflambda_mkpkp, kfsigma0_mkpkp;
...
if (evt.n(pid::kplus) > 1)
{
    ...
    {
        /// g p --> K+ K+ K- X
        cp_km0 = evt.charged_particle(pid::kminus, 0);
        km0 = cp_km0.four_momentum();

        mkpkpkm = beam + target - kp0 - kp1 - km0;
        hmkpkpkm_m.fill(mkpkpkm.invariant());

        // add phi cut

        double dm = 0.4999 * (pdg::mass::lambda + pdg::mass::sigma0);
        vector<charged_particle> cp_kpkpkm { cp_kp0, cp_kp1, cp_km0 };

        if (abs(mkpkpkm.invariant() - pdg::mass::lambda) < dm)
        {
            /// g p --> [Lambda] K+ K+ K-
            kinfit_output_t kfit_lambda = kinematic_fit(tag, pdg::mass::proton, cp_kpkpkm, pdg::mass::lambda, "g12");

            kfkp0 = kfit_lambda.parts[0];
            kfkp1 = kfit_lambda.parts[1];
            kfkm0 = kfit_lambda.parts[2];
            ...
            hkflambda_pull0_km0.fill(kfit_lambda.pulls[2][0]);
            hkflambda_pull1_km0.fill(kfit_lambda.pulls[2][1]);
            hkflambda_pull2_km0.fill(kfit_lambda.pulls[2][2]);
