...
double AlphaTrack(const TLorentzVector &__p4)
{
    ...
    double phi = __p4.Phi() + (pi/6.);
    int sector = floor((3./pi) * (phi<0 ? phi+2*pi : phi)) + 1;
...
double LambdaTrack(const TLorentzVector &__p4)
{
    ...
    return asin((cos(alpha)*__p4.Y() - sin(alpha)*__p4.X()) / __p4.P());
}
//_____________________________________________________________________________
...
double PhiTrack(const TLorentzVector &__p4)
{
    ...
    return acos( __p4.Pz() / (__p4.P() * cos(lambda)) );
}
...
TMatrixD CorrectCLAS_V(
    const TMatrixD&                    __VTrack,
    ...
    double __res_scale = -1 )
{
    ...
    {
        cerr << "Error! <CorrectCLAS_V> Tracking Variance matrix passed to "
    ...
    {
        cerr << "Error! <CorrectCLAS_V> Tracking Variance matrix size is NOT"
             ...
             << __VTrack.GetNrows() << " rows and columns." << endl;
        abort();
    }
    ...
    {
        cerr << "Error: Experiment " << __xid << " not known, use either g10a, g11a, g12 or g13" << endl;
        abort();
    }

   double e_gamma_res;
        if (__xid == "g10a") { e_gamma_res = 3.776; }
   else if (__xid == "g11a") { e_gamma_res = 4.018; }
   else if (__xid == "g12" ) { e_gamma_res = 5.715; }
   else if (__xid == "g13a") { e_gamma_res = 2.655; }

   e_gamma_res *= 0.001/sqrt(3.);

    if (__r_scale > 0 && __r_scale > 0)
    {
        r_scale = __r_scale;
        res_scale = __res_scale;
    }
    else if (__xid == "g10a")
    {
        cell =6;
    ...
    else if (__xid == "g12")
    {
        ...
        //res_scale =2.83 + 0.1; // sigma(pull 0 [p]K+K-) = 0.501
        //res_scale =2.83; // sigma(pull 0 [p]K+K-) = 0.7
        //res_scale =2.83 - .13; // sigma(pull 0 [p]K+K-) = 0.529
        //p_scale = 1.94;
        //r_scale = 1.3;
    ...
    for (int i = 0; i < numParts; i++)
    {
        ...
        if (__p_names[i]!="gamma" && __p_names[i]!="n")
        {
            if (__multi==false)
            {
                static bool first_call = true;

                static double pPars[2][6][15][3];
                static double lamPars[2][6][15];
                static double phiPars[2][6][15];
                ...
                if (__p4[i].M() < .7 || __p_names[i]=="pi+" || __p_names[i]=="pi-")
                {
                    part_index = 1; // use pion parameters
                }
                else
                {
                    part_index = 0; // use proton parameters
                }
            ...
            else /* if (__multi==true) */
            {
                /// assume the r_scale and res_scale are set for kaons
                /// already. The corrections here are for protons.
                if (__p_names[i]=="p")
                {
                    r_scale *= 1.;
                    res_scale *= 1.;
...
bool ReadInResParams(double __pPars[2][6][15][3],double __lamPars[2][6][15],
  ...
  String fileName_base = "/home/goetz/local/src/kinfit/parms/";
  String fileName;
  String p_names[2];
  p_names[0] = "p";
  p_names[1] = "pi";
