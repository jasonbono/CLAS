...
private:
  ...
  void _SetMissingParticleV(const TMatrixD &__missingV,
				 const TVectorD &__x);
public:
  // Create/Copy/Destroy:
  Kstream();
  ...
  inline void FitInput(const std::vector<String> &__p_names,double __ephot,const std::vector<TLorentzVector> &__p4,
		       const TMatrixD &__VMat,double __targMass = -1.){
    if(__targMass > 0.) _targetMass = __targMass;
    _ephot_in = __ephot;
    _p4in = __p4;
  ...
  inline const TMatrixD& GetMissingVMat() const {
    return _Vmiss;
  }
