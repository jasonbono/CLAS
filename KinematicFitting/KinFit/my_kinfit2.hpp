// March 26 2012: this combines my_kinft.hpp and kinfit.hpp into one header file
...
kinfit_output_t my_kinematic_fit(
    const fourVec& tag,
    const double& target_mass,
    std::vector<clasParticle>& input_particles,
    const double& missing_mass,
    const double& invariant_mass,  //jb this will be for the mass of the cascade
    ...
    const threeVec vertex)
{
  ...
  double resscale = -1; 
    ...
    bool multi = true;
    bool mc = false;
    //jb std::vector<bool> set(nparts, false);
	std::vector<bool> set(nparts);  //jb
	set[0] = false; //jb
	set[1] = false; //jb
	set[2] = true;  //jb include pion in invarent mass constrain
    ...
    /// do the fit.
    bool include = true; //jb include the missing lambda in 
               
    //jb fit.Fit(missing_mass);
    fit.Fit(missing_mass, set, include, invariant_mass); //jb
