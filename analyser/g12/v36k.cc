...
April 9 v36k:  added mukeshes pcor:  failed
...
#include "my_g12_pcor_alt.h" //jb v36k
...
void printLabels()
{
 ...
 cout << ++nlab << "\toldbeam energy{beam_woE}"<<endl;
 cout << ++nlab << "\tmass of (mmkpkp_woE) {mmkpkp_woE}"<<endl;
 cout << ++nlab << "\tmass of (mmkpkppim_woE) {mmkpkppim_woE}"<<endl;
...
read_pcor( pcor_array );
 ...
 double oldbeamp = -100, beamp = -100.0, psbeamp = -100.0, mmbeamp = -100.0, fixbeamp = -100.0;
 ...
 beamp = corrected_beam_energy(my_event.run(), oldbeamp);
 
 //make a 4vec from the old beam for later checking
 fourVec oldbeam; 
 oldbeam.set(oldbeamp,threeVec(0.0,0.0,oldbeamp));
 //---------------------------------------------------------------------------------------------------------------------------------------
 ...
 if(st&&bb)  //jb v35k putting the filter back
 {
  ...
  output << oldbeamp << " "; //jb v35k beam_woE
  output << ~(oldbeam + target - kp1 - kp2) << " " <<~(oldbeam + target-kp1-kp2 - pim)<<" "; //jv v35k mm_woE
