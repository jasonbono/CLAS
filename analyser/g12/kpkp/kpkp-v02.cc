...
Septemer 30: kpkp-v01 took out even more things that arent needed.

September 28: came from 2kppim-v00 as a template.  this analyzer is striped down to compute things for kpkp topology. before deleeting will do ///5 ///6 ///7 etc using 1-0
...
void printLabels(){
 ...
 ///1 cout << ++nlab << "\tmass of (mmkpkppim_woecor) {mmkpkppim_woecor}"<<endl;
 ///2 cout << ++nlab << "\tmass of (mmkpkp_woeloss) {mmkpkp_woeloss}"<<endl;
 ///3 cout << ++nlab << "\tmass of (mmkpkppim_woeloss) {mmkpkppim_woeloss}"<<endl;
 cout << ++nlab << "\tmass of (mmkpkp_woecoreloss) {mmkpkp_woecoreloss}"<<endl;
 ///4 cout << ++nlab << "\tmass of (mmkpkppim_woecoreloss) {mmkpkppim_woecoreloss}"<<endl;
...
///5 cout << ++nlab << "\t(mass of proton pi-){mppim}"<<endl;
 cout << ++nlab << "\t{tofbetakp1}"<<endl;
 cout << ++nlab << "\t{tofbetakp2}"<<endl;
// cout << ++nlab << "\t{tofbetapim}"<<endl;
 cout << ++nlab << "\t{betakp1}"<<endl; //20
 cout << ++nlab << "\t{betakp2}"<<endl;
///6 cout << ++nlab << "\t{betapim}"<<endl;
 cout << ++nlab << "\t{sctkp1}"<<endl;
 cout << ++nlab << "\t{sctkp2}"<<endl;
///7 cout << ++nlab << "\t{sctpim}"<<endl;
 cout << ++nlab << "\t{sclkp1}"<<endl;
 cout << ++nlab << "\t{sclkp2}"<<endl;
///8 cout << ++nlab << "\t{sclpim}"<<endl;
 cout << ++nlab << "\t{tofmasskp1}"<<endl;
 cout << ++nlab << "\t{tofmasskp2}"<<endl;
///9 cout << ++nlab << "\t{tofmasspim}"<<endl;
 cout << ++nlab << "\t{stvkp1}"<<endl;
 cout << ++nlab << "\t{stvkp2}"<<endl;
///0 cout << ++nlab << "\t{stvpim}"<<endl;

 cout << ++nlab << "\t{goodSTkp1}"<<endl; //30 //jb v16
 cout << ++nlab << "\t{goodSTkp2}"<<endl;  //jb v16
///1 cout << ++nlab << "\t{goodSTpim}"<<endl;  //jb v16

 cout << ++nlab << "\t{stid_kp1}"<<endl;  //jb v17
 cout << ++nlab << "\t{stid_kp2}"<<endl;  //jb v17
///2 cout << ++nlab << "\t{stid_pim}"<<endl;  //jb v17

 // distances
///3 cout << ++nlab << "\t(distance between K+1, pi-){dkp1pim}"<<endl;
///4 cout << ++nlab << "\t(distance between K+2, pi-){dkp2pim}"<<endl;
...
///5 cout << ++nlab << "\tt energy transfer(kp1+kp2+pim - beam){tkpkppim}"<<endl;
///6 cout << ++nlab << "\tt energy transfer(kp1+pim - beam){tkp1pim}"<<endl;
///7 cout << ++nlab << "\tt energy transfer(kp2+pim - beam){tkp2pim}"<<endl;
...
///8 cout << ++nlab << "\tmissing mass square of (kp1+pim){mm2kp1pim}"<<endl;
///9 cout << ++nlab << "\tmissing mass square of (kp2+pim){mm2kp2pim}"<<endl;
///0 cout << ++nlab << "\tmissing mass square of (kp+kp+pim){mm2kpkppim}"<<endl;
...
///1 cout << ++nlab << "\tmass of (k+1 pi-) {mkp1pim}"<<endl;
///2 cout << ++nlab << "\tmass of (k+2 pi-) {mkp2pim}"<<endl;
 //missing masses
 cout << ++nlab << "\tmass of (mmkp1){mmkp1}"<<endl;
 cout << ++nlab << "\tmass of (mmkp2) {mmkp2}"<<endl;
 cout << ++nlab << "\tmass of (mmkpkp) {mmkpkp}"<<endl;
///3 cout << ++nlab << "\tmass of (mmkpkppim) {mmkpkppim}"<<endl;
 //momentums
 cout << ++nlab << "\tkp1 momentum{pkp1}"<<endl;
 cout << ++nlab << "\tkp2 momentum{pkp2}"<<endl;
///4 cout << ++nlab << "\tkp2 momentum{ppim}"<<endl;
///5 cout << ++nlab << "\tmiss x{missx}"<<endl;
///6 cout << ++nlab << "\tmiss y{missy}"<<endl;
///7 cout << ++nlab << "\tmiss z{missz}"<<endl;
///8 cout << ++nlab << "\tmiss momentum{missp}"<<endl; 
///9 cout << ++nlab << "\tmiss momentum{misse}"<<endl; 
 //mandalstam varaible t
///0 cout << ++nlab << "\tt prime{tp}"<<endl;
 cout << ++nlab << "\tt prime{tpkp1}"<<endl;
 //replot invarent mass with the the constrained masses
///EXTRA1 cout << ++nlab << "\tMass(Cascade, K+1){mcaskp1}"<<endl;
///EXTRA2 cout << ++nlab << "\tMass(Cascade, K+2){mcaskp2}"<<endl;
///1 cout << ++nlab << "\tMass(Cascade, pim){mcaspim}"<<endl;
///2 cout << ++nlab << "\tMass(lambda, pim){mlampim}"<<endl;
///3 cout << ++nlab << "\tMass(Cascade, pim) fix beam method{mfixcaspim}"<<endl;
///4 cout << ++nlab << "\tbeam energy fix beam method{fixbeam}"<<endl;  //fixbeam is the {kp kp pim (Xi0)} hypothisis: fixbeam2 is the {kp kp (Xi-)} hypothisis.
 cout << ++nlab << "\tMass(Cascadem, kp1){mcasmkp1}"<<endl;
 cout << ++nlab << "\tMass(Cascadem, kp2){mcasmkp2}"<<endl;
///5 cout << ++nlab << "\tMass(Cascadem - pim){mcasmMpim}"<<endl;  //jb v15 
...
///6 cout << ++nlab << "\tMissing Mass(3pi){mm3pi}"<<endl;
///7 cout << ++nlab << "\tMissing Mass(K+1, pip, pim){mmkp1pippim}"<<endl;
///8 cout << ++nlab << "\tMissing Mass(K+2, pip, pim){mmkp2pippim}"<<endl; 
 //jbv07_2 leaves for 2 particle for k+ replaced with pi+
///EXTRA3 cout << ++nlab << "\tMissing Mass(2pip){2pip}"<<endl;
///EXTRA4 cout << ++nlab << "\tMissing Mass(K+1, pip){mmkp1pip}"<<endl;
///EXTRA5 cout << ++nlab << "\tMissing Mass(K+2, pip){mmkp2pip}"<<endl;
...
///EXTRA6 cout << ++nlab << "\tMissing Mass(pip1){mmpip1}"<<endl;
///EXTRA7 cout << ++nlab << "\tMissing Mass(pip2){mmpip2}"<<endl;

///9 cout << ++nlab << "\tMass(K+1, pip, pim){mkp1pippim}"<<endl;
///0 cout << ++nlab << "\tMass(K+2, pip, pim){mkp2pippim}"<<endl;
///1 cout << ++nlab << "\tMass(pip1, pim){mpip1pim}"<<endl;
///2 cout << ++nlab << "\tMass(pip2, pim){mpip2pim}"<<endl;
///3 cout << ++nlab << "\tMass(pip1, pip2, pim){m3pi}"<<endl;
 //jbv07_3 the missing mass of pip + pim system
///4 cout << ++nlab << "\tMissing Mass(pip1 + pim){mmpip1pim}"<<endl;
///5 cout << ++nlab << "\tMissing Mass(pip2 + pim){mmpip2pim}"<<endl;
/* HUGE1
 // kf
...
{
 int nReturnValue = 0;
 //SET THE TOPOLOGY HERE
 if (my_event.N(KPlus)!=2 || my_event.N(KMinus)!=0 )
 {
  return (0);
 }
   
 //declare basics and set beam and target four vectors 
 fourVec kp1,kp2, kp0; //kp0 needed for switching kp1 and kp2
 fourVec beam,target(PROTON_MASS,threeVec(0.0,0.0,0.0));
 double beamp_woecor = -100, beamp = -100.0;
 ...
 fourVec kp1_woeloss, kp2_woeloss;  //jb v12 made this for showing missing mass spectrum without eloss corrections.
 ...
 // get the photon {id,energy (w/o corections) and timing} from the tagr
 if(tagr)
 {
  eid = tagr->e_id;
 ...
 beamp = beamp_woecor; //initiize the beam energy without any corrections
 ...
 if(corr)   
 {
  ...
  if(datatype) //only correct the beam for the Data and not the MC events.
  {
 ...
 clasParticle cpkp0, cpkp1, cpkp2;
...
// if((Vert.x()*Vert.x()+Vert.y()*Vert.y())<=4) targXY=1;  //\\ this too
 if(abs(my_event.scVtime()-my_event.stVtime())<=2) st=1; //\\ this cut should be relaxed then the data should be looked at to determine where to place cuts
 //jb v35k adding the beam energy filter: this is the energy needed for production of k+ k+ Xi- from gamma and proton
 int bb=0;
 if(beamp > 2.373) bb=1;  //keeping the same for kpkp as kpkppim, thats because the threshold energy is the same
 ...
 if(st&&bb)  //jb v35k putting the filter back
 {
  ...
  //jb v35k beam_woecor
  output << beamp_woecor << " "; 
  output << ~(beam_woecor + target - kp1 - kp2) << " "; 
///1  output << ~(beam_woecor + target-kp1-kp2 - pim) << " ";
///2  output << ~(beam + target - kp1_woeloss - kp2_woeloss) << " ";
///3  output << ~(beam + target-kp1_woeloss-kp2_woeloss - pim_woeloss)<< " ";
  output << ~(beam_woecor + target - kp1_woeloss - kp2_woeloss) << " "; 
///3  output << ~(beam_woecor + target-kp1_woeloss-kp2_woeloss - pim_woeloss)<< " ";
...
///4  output<<mppim<<" ";  
  ...
  output << cpkp1.beta() <<" ";
  output << cpkp2.beta() <<" ";
///5  output << my_event.cp(PiMinus, 1).beta() <<" ";
  output << cpkp1.Beta() <<" ";
  output << cpkp2.Beta() <<" ";  //20
///6  output << my_event.cp(PiMinus, 1).Beta() <<" ";
  //sctime and scpath
  output << cpkp1.sc_time() <<" ";
  output << cpkp2.sc_time() <<" ";
///7  output << my_event.cp(PiMinus, 1).sc_time() <<" ";
  output << cpkp1.scPathLen() <<" ";
  output << cpkp2.scPathLen() <<" ";
///8  output << my_event.cp(PiMinus, 1).scPathLen() <<" ";
  //TOF mass
  output << cpkp1.mass() <<" ";
  output << cpkp2.mass() <<" " ;
///9  output << my_event.cp(PiMinus, 1).mass() <<" ";
  //check st_vtime for different particles
  output << cpkp1.stVtime() <<" ";
  output << cpkp2.stVtime() <<" "; 
///0  output << my_event.cp(PiMinus, 1).stVtime() <<" ";
  //check st 
  output << cpkp1.isSThit() <<" ";  	//jb v16
  output << cpkp2.isSThit() <<" ";   //jb v16
///1  output << my_event.cp(PiMinus, 1).isSThit() <<" "; //jb v16	
  output << cpkp1.st_id() <<" ";  	//jb v17
  output << cpkp2.st_id() <<" ";   //jb v17
///2  output << my_event.cp(PiMinus, 1).st_id() <<" "; //jb v17 
...
///5  output<< (kp1+kp2+pim-beam).lenSq() <<" ";
///6  output << (kp1+pim-beam).lenSq() <<" ";
///7  output << (kp2+pim-beam).lenSq() <<" ";
  output << (beam + target - kp1 - kp2).lenSq() << " ";
  output << (beam + target - kp1).lenSq() << " ";
  output << (beam + target - kp2).lenSq() << " ";  
///8  output << (beam + target - kp1-pim).lenSq() << " ";
///9  output << (beam + target - kp2-pim).lenSq() << " ";  
///0  output << (beam + target - kp1 - kp2-pim).lenSq() << " ";  
...
///3  output << ~(beam + target - kp1 - kp2 - pim) << "";
  
  //momentums
  output << kp1.r() <<" ";
  output << kp2.r() <<" ";
///4  output << pim.r() <<" ";
///5  output << (beam + target - kp1 - kp2 - pim).x() << " ";
///6  output << (beam + target - kp1 - kp2 - pim).y() << " ";
///7  output << (beam + target - kp1 - kp2 - pim).z() << " ";
///8  output << (beam + target - kp1 - kp2 - pim).r() << " ";
  
  //missing energy
///9  output << (beam + target - kp1 - kp2 - pim).t() << " ";
  
  //mandalstam variable t to kp1 
///0  output << -((kp1 + kp2 + pim - beam).lenSq() - tMin) << " ";
  tMinkp1  = tmin( beamp, 0.0, PROTON_MASS, KP_MASS, CASCADE_MASS);
  output << -((kp1 - beam).lenSq() - tMinkp1) << " ";

  //constrain cascade mass and replot the k+ cascade masses
  ...
  //replot invarent mass with the the constrained masses
///EXTRA1  output << ~(cascade + kp1) <<" "; 
///EXTRA2  output << ~(cascade + kp2) <<" ";
///1  output << ~(cascade + pim) <<" ";
///2  output << ~(lambda+pim) <<" "; 
///3  output << ~(fixbeam+target-kp1-kp2) <<" ";
///4  output << fixbeamp <<" ";  
  output << ~(cascadem + kp1) <<" ";
  output << ~(cascadem + kp2) <<" ";
///5  output << ~(cascadem - pim) <<" ";
  ...
  //more masses
///6  output << ~(beam+target-pip1-pip2-pim) << " ";
///7  output << ~(beam+target-kp1-pip2-pim) <<" ";
///8  output << ~(beam+target-pip1-kp2-pim) << " ";
///EXTRA3  output << ~(beam+target-pip1-pip2) << " ";
///EXTRA4  output << ~(beam+target-kp1-pip2) << " ";
///EXTRA5  output << ~(beam+target-kp2-pip1) << " ";
  output << ~(beam+target-kp1) << " ";
  output << ~(beam+target-kp2) << " ";
///EXTRA6  output << ~(beam+target-pip1) << " ";
///EXTRA7  output << ~(beam+target-pip2) << " ";
///9  output << ~(kp1+pip2+pim) << " ";
///0  output << ~(kp2+pip1+pim)<< " ";
///1  output << ~(pip1+pim) << " ";
///2  output << ~(pip2+pim) << " ";
///3  output << ~(pip1+pip2+pim) << " ";
///4  output<< ~(beam+target-pip1-pim) << " ";  //jbv07_3
///5  output << ~(beam+target-pip2-pim) << " ";  //jbv07_3
