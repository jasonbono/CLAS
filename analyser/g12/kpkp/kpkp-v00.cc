...
///4  output<<mppim<<" ";
  ...
  if(kp1.r()>kp2.r())  //if kp1 is faster than kp2
  {
...
///2   output << my_event.cp(PiMinus, 1).st_id() <<" "; //jb v17 
   ...
   cpkp1 = my_event.cp(KPlus, 1); //the faster kaon is kp1
  ...
  else 
  {
   ...
   output << my_event.cp(KPlus, 2).mass() << " ";
   output << my_event.cp(KPlus, 1).mass() << " ";
   //st_vtime for different particles
   output << my_event.cp(KPlus, 2).stVtime() << " ";
   output << my_event.cp(KPlus, 1).stVtime() << " "; 
  
   cpkp1 = my_event.cp(KPlus, 2); //the faster kaon is kp1
