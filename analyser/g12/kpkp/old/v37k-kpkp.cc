...
July 5 2012:  this is the first attempt to convert the analyzer to kpkp. this was taken from v37k and will now be called kpkp-v00.cc kpkp-v01.cc etc etc.  look for 'kpkp-v00' for edits.
the other topologys will the the analyzer changed as well. ie v37k.cc will be converted to 2kppim-v00.cc etc.
...
int ProcessEvent(clasEvent &my_event,int verbose, int IFLAG)
{
 ...
 if (my_event.N(KPlus)!=2)   //kpkp-v00
 {
  return (0);
 }
 ...
 //kpkp-v00 pim_woeloss=my_event.cp(PiMinus,1).p();
 ...
 //kpkp-v00 pim=my_event.cp(PiMinus,1).p();
 ...
 if(st&&bb)  //jb v35k putting the filter back
 {
  ...
  output << ~(beam_woecor + target-kp1-kp2 - pim) <<" "; //jb v35k mm_woecor
  ...
  //kpkp-v00 output<<my_event.N(KPlus)<<" "<<my_event.N(PiMinus)<<" "<<my_event.N(KMinus)<<" "<<my_event.N(Proton)<<" "<<my_event.N(PiPlus)<<" ";
