//OCT 18 added all PIangles, n, z, x, y.... plan on taking  cosine of them in analyser once i demonstrate that my ana agrees witht the add branch results from the root macro
...
cout << ++nlab << "\t(K+2) lab angle{kp2labtheta}"<<endl;
 ...
 //X rest frame angles
  ...
  //JB\\v04 PIanle to norm of procution plane
...
//get the meson system 4-vector
  ...
  //JB\\v04 for adding PIangleANA
          fourVec pim_1;
	  lorentzTransform L9;  
	  L9.set(X);
	  pim_1.set(pim.t(),pim.V());
	  pim_1 *=L9;
  //JB\\v05 adding all angles for pi and lambda
	  fourVec lambda_1;
	  lambda_1.set(lambda.t(),lambda.V());
	  lambda_1 *=L9;
  //JB\\v05 defining the lab coords
	  threeVec xlab=(1.0,0.0,0.0);
	  threeVec ylab=(0.0,1.0,0.0);
	  threeVec zlab=(0.0,0.0,1.0);
...
//JB\\v05 output<<(Cross - pim_1.V()).cosTheta()<<endl;
...
output<<(Cross - lamb_1.V()).cosTheta()<<" ";
output<<(xlab - lamb_1.V()).cosTheta()<<" ";
output<<(ylab - lamb_1.V()).cosTheta()<<" ";
output<<(zlab - lamb_1.V()).cosTheta()<<endl;
