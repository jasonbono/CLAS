...
october 25 v09:  will attempt to totaly switch over to root variables for the angles. can revert back to v08 if the chages do not work out

october 24:  v08
need to change the calculation of angles because they are wrong. can step into .theta() with ddd to find where it is defined. can also find claspack in this way.
also am taking out the mmbeam stuff because that was done in cooking. care must be taken when replacing mmbeamp with beamp.
also puneet advised me to change over to root variables. not sure quite yet what to do.
...
//jb=>puneet additional ROOT libraries included /////////////////////////////////////////////////////////
#include "TLorentzVector.h"  // quotes are needed when the library is not in the standard path
                             // the library header file name must have the .h extension 
#include "<TVector3.h" //jb v09
...
int ProcessEvent(clasEvent &my_event,int verbose, int IFLAG)
{
  ...
  int eid = 0;
   if(tagr){
     eid = tagr->e_id;
     beamp = tagr->erg;           //\\ v08 this is the correct energy
   }
 ...
 evtno=my_event.event();
  ...
  //do eloss and mommentum correction
    ...
    /*                           // jb v08 took all this out because its done in cooking
    if(datatype) {
    ...
    //jb v07   my_event.Pcor(); 

*/
  }

  //jb v08  beam.set(mmbeamp,threeVec(0.0,0.0,mmbeamp));
  beam.set(beamp,threeVec(0.0,0.0,beamp));  //jb v08
...
//jb v08  output<<mmbeamp<<" "<<~(beam+target)<<" "<<(kp1+kp2-beam).lenSq()<<" ";    //jb v < 05
  output<<beamp<<" "<<~(beam+target)<<" "<<(kp1+kp2-beam).lenSq()<<" ";    //jb v08
...
//jb v09 doing it the root way.===================================================================
  TLorentzVector my_X(X.x(), X.y(), X.z(), X.t());
  TLorentzVector my_pim(pim.x(), pim.y(), pim.z(), pim.t());
  TLorentzVector my_lambda(lambda.x(), lambda.y(), lambda.z(), lambda.t());
  TVector3 my_Xboost;
  my_Xboost = -my_X.BoostVector();
  my_pim.Boost(my_Xboost);	  
  my_lambda.Boost(my_Xboost);
  //and storing the momentum as a TVector3...
  TVector3 Tmy_pim;
  TVector3 Tmy_lambda;
  Tmy_pim.SetXYZ(my_pim(0),my_pim(1),my_pim(2));
  Tmy_lambda.SetXYZ(my_lambda(0),my_lambda(1),my_lambda(2));
  //now store the lab coords as TVector3
  TVector3 Tmy_xlab;
  TVector3 Tmy_ylab;
  TVector3 Tmy_zlab;
  Tmy_xlab.SetXYZ(1.0,0.0,0.0);
  Tmy_ylab.SetXYZ(0.0,1.0,0.0);
  Tmy_zlab.SetXYZ(0.0,0.0,1.0);
  //==============================================================================================  
  //now the 3 and 4 momenta are stored so that the angles can be taken wrt n,x,y,z.  
  //n is defined later in this code and a TVector3 will have to be defined from it.
...
*/
//jb v09 =================================================================================================
// fill the normal to the production plane as Tmy_n 

  TVector3 Tmy_n;
  Tmy_n.SetXYZ(Cross(0),Cross(1),Cross(2));
  
  // now can output variables
  output << cos(Tmy_pim.Angle(Tmy_n))<<" ";
  output << cos(Tmy_pim.Angle(Tmy_xlab))<<" ";
  output << cos(Tmy_pim.Angle(Tmy_ylab))<<" ";
  output << cos(Tmy_pim.Angle(Tmy_zlab))<<" ";
  output << cos(Tmy_lambda.Angle(Tmy_n))<<" ";
  output << cos(Tmy_lambda.Angle(Tmy_xlab))<<" ";
  output << cos(Tmy_lambda.Angle(Tmy_ylab))<<" ";
  output << cos(Tmy_lambda.Angle(Tmy_zlab))<<endl;
  // ====================================================================================================
