// OCT 18 now added 1 angle branch PIangle_ANA

// Oct 18. version 3 totaly took out the eloss and p correcitons.

//only took out some ps beam things. here for changing eloss and momentum corrections see version 2.

//October 10 2011.  edited the g11 analyzer to accomodate g12 data
// look for //JB\\
...
void printLabels()
{
  ...
  cout << ++nlab << "\trun number{run}"<<endl;  //1
  cout << ++nlab << "\tevt number{evt}"<<endl;  //2
  cout << ++nlab << "\tsc Vtime{scv}"<<endl; //3
  cout << ++nlab << "\tst Vtime{stv}"<<endl; //4
 cout << ++nlab << "\tvtim should be same as sc Vtime{vtime}"<<endl; //5
 cout << ++nlab << "\tmvrt x{vx}"<<endl; //6
 cout << ++nlab << "\tmvrt y{vy}"<<endl; //7
 cout << ++nlab << "\tmvrt z{vz}"<<endl;  //8  ok
 cout << ++nlab << "\tnumber of K+{nkp}"<<endl;  //9
 cout << ++nlab << "\tnumber of pi-{npim}"<<endl;  //10
 cout << ++nlab << "\tnumber of K-{nkm}"<<endl;
 cout << ++nlab << "\tnumber of proton{np}"<<endl;
 cout << ++nlab << "\tnumber of pi+{npip}"<<endl; //12 ok
 cout << ++nlab << "\t{kfit probability, confidence level}{cl}"<<endl; //13
 cout << ++nlab << "\t{kfit chisq}{chi2}"<<endl; //14
 cout << ++nlab << "\t{kfit beam}{bfit}"<<endl; //15 ok
 cout << ++nlab << "\t{kfit mm}{fitmmkkpim}"<<endl; //16 ok
 cout << ++nlab << "\t{kfit mmkpkp}{fitmmkpkp}"<<endl; //17 ok
 cout << ++nlab << "\t{kfit mmkp1}{fitmmkp1}"<<endl; //18
 cout << ++nlab << "\t{kfit mmkp2}{fitmmkp2}"<<endl; //19
 cout << ++nlab << "\t{kfit mmpim}{fitmmpim}"<<endl; //20
 cout << ++nlab << "\t{kfit m(kp1+pim)}{fitmkp1pim}"<<endl; //21
 cout << ++nlab << "\t{kfit m(kp2+pim)}{fitmkp2pim}"<<endl;  //22
 cout << ++nlab << "\t(mass of proton pi-){mppim}"<<endl; //23 ok
 cout << ++nlab << "\t{tofbetakp1}"<<endl;  //24
 cout << ++nlab << "\t{tofbetakp2}"<<endl;  //25
 cout << ++nlab << "\t{tofbetapim}"<<endl;
 cout << ++nlab << "\t{betakp1}"<<endl;
 cout << ++nlab << "\t{betakp2}"<<endl;
 cout << ++nlab << "\t{betapim}"<<endl; //30
 ...
 cout << ++nlab << "\tbeam energy{beam}"<<endl;     //line 
 //JB\\cout << ++nlab << "\tbeam energy from ps tagger correction{psbeam}"<<endl;               // omit_1  47th write out
 cout << ++nlab << "\tsqrt(s){w}"<<endl;
 cout << ++nlab << "\tt energy transfer(kp1+kp2 - beam){tkpkp}"<<endl;
 cout << ++nlab << "\tt energy transfer(kp1+kp2+pim - beam){tkpkppim}"<<endl;  //50
 ...
 cout << ++nlab << "\tt energy transfer(kp2+pim - beam){tkp2pim}"<<endl;
  ...
  //JB\\  cout << ++nlab << "\tmissing mass square of using ps beam (K+K+) {psmm2kpkp}"<<endl;       //2
 ...
 //JB\\ cout << ++nlab << "\tmissing mass square of (kp+kp+pim)using ps beam{psmm2kpkppim}"<<endl;       //3
 ...
 //JB\\   cout << ++nlab << "\tMass(Cascade, pim) ps method{mpscaspim}"<<endl;          //4
  ...
  cout << ++nlab << "\t(Kp2)(kp2+xm) rest angle{kp2hphi}"<<endl; // 108 ok
  //Y2 rest frame angles
  cout << ++nlab << "\t(Kp1) (kp1+xm) rest angle{kp1htheta}"<<endl;  //109
  cout << ++nlab << "\t(Kp1) (kp1+xm) rest angle{kp1hphi}"<<endl;  //110  ok
 //X rest frame angles
  cout << ++nlab << "\t(pi) Xi rest angle{pihtheta}"<<endl;  //111
  cout << ++nlab << "\t(pi) Xi rest angle{pihphi}"<<endl;    //112
 	
  //JB\\ PIanle to norm of procution plane
  cout << ++nlab << "\t(pi) Xi norm angle{PIangle_ANA}"<<endl;  //113    ok!
...
int ProcessEvent(clasEvent &my_event,int verbose, int IFLAG)
{
  ...
  int evtno=0;
  double tMin;
  tagr_t *tagr = my_get_photon_tagr(TAGR, TBID); //\\//\\// can see this from bosdump -GTAGR -GTBID
  //\\//\\\//\\//\\ should be fine thoe
  int eid = 0;
   if(tagr){
     eid = tagr->e_id;
 ...
 //    my_event.eLoss("g11a", IFLAG);//\\//\\//\\//\\/ look in clas pack *see scratch notes*
    //if(datatype) {  //\\//\\//\ this means only runs on real data. GET RID OF THIS i think
      //my_event.momentumCorrection();
      //  psbeamp=my_event.lgTAGRcorrection(beamp, eid); //JB\\ looks like i should get rid of theese
      //if(runno>44109){
 ...
 //      mmbeamp=beamp;
 //    }
 //    my_event.Pcor(); //\\//\\/ this is the g11 equivilant of what the odu student wrote for g12 but may not be in claspac
 //  }
 ...
 //Now check with kinematic fitting
  //\\//\\//\\//\\// have to change this *see scratch*
  clasKineFit fit;
  clasParticle l[3]; 
  ...
  //\\//\\//\\//\\//\\//\\ make sure you know what theese are, dont confuse scintilator with start counter!!!!!!!!
  float scv=my_event.scVtime();
  float stv=my_event.stVtime();
  threeVec Vert = my_event.V();

  int targZ=0, targXY=0, st=0;
  if(abs(Vert.z()+10)<=20) targZ=1;  //\\//\\// he abandoned this cut anyway
  if((Vert.x()*Vert.x()+Vert.y()*Vert.y())<=4) targXY=1; //\\//\\// same here
  if(abs(my_event.scVtime()-my_event.stVtime())<=3) st=1;  // this should be relaxed then the data should be looked at to determine where to place cuts

  float mm2kpkp=(beam + target - kp2 - kp1).lenSq();
  if(st){
  nReturnValue = 1;
  output<<runno<<" "<<evtno<<" ";  //1
...
//now plot the masses
  //JB\\output<<mmbeamp<<" "<<psbeamp<<" "<<~(beam+target)<<" "<<(kp1+kp2-beam).lenSq()<<" ";              //omit_1
  ...
  //JB\\  output << (psbeam + target - kp1 - kp2).lenSq() << " ";                                //omit_2
  ...
  //JB\\ output << (psbeam + target - kp1 - kp2-pim).lenSq() << " ";                           //omit_3     
  ...
  tMin = tmin(beamp,0.0,PROTON_MASS,~(kp1 + kp2 + pim),CASCADE0_MASS);//\\//\\check definition
  ...
  //JB\\ output<<~(cascade + kp1) <<" "<<~(cascade + kp2)<<" "<<~(cascade + pim)<<" "<<~(lambda+pim)<<" "<<~(pscascade + pim)<<" "<<~(fixbeam+target-kp1-kp2)<<" "<<fixbeamp<<" ";    //4
  output<<~(cascade + kp1) <<" "<<~(cascade + kp2)<<" "<<~(cascade + pim)<<" "<<~(lambda+pim)<<" "<<~(fixbeam+target-kp1-kp2)<<" "<<fixbeamp<<" "; //JB\\oct13
...
//now we are in the X (cascade*) helicity rest frame , output the 2-body decay angles of Pi- in X rest frame
  ...
  if(fabs(ctpim)>0&&fabs(phipim)>0){
    //JB\\ removed the endl and but " " in there place for BOTH of the output statements below. this is so i can add more to output.
    output<<ctpim<<" "<<phipim<<" ";
  }
  else{
    output<<" -10 "<<" "<<" -10 "<<" ";
...
///JB\\ now get angle between pim_1 and normal and print it out	  
///JB\\ the previous boosts did not act on theese quantities
///JB\\ pim_1 is in the X resframe with z aligned with beam, and cross is in cm with z aligned with beam	  
output<<(Cross - pim_1.V()).cosTheta()<<endl;
