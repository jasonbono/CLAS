...
void printLabels()
{
 ...
 cout << ++nlab << "\t{kfit mmkp2}{fitmmkp2}"<<endl; //20
 ...
 //X rest frame angles
  ...
  cout << ++nlab << "\t(pi) Xi norm angle{PIangle_ANA}"<<endl; 
...
int ProcessEvent(clasEvent &my_event,int verbose, int IFLAG)
{
 ...
 //Now check with kinematic fitting
  ...
  if(st){
  nReturnValue = 1;
  output<<runno<<" "<<evtno<<" ";  //1,2
  output<<my_event.scVtime()<<" "<<my_event.stVtime()<<" "<<my_event.vtime()<<" ";  //3,4,5
  output<<Vert.x()<<" "<<Vert.y()<<" "<<Vert.z()<<" "; //6,7,8
  output<<my_event.N(KPlus)<<" "<<my_event.N(PiMinus)<<" "<<my_event.N(KMinus)<<" "<<my_event.N(Proton)<<" "<<my_event.N(PiPlus)<<" "; //8,9,10,11,12  ok
  output << fit.prob() << " "<<fit.chisq()<<" ";  //13,14
  output<<fit.beamEnergy()<<" "<<~(beamOut+target-kp1Out-kp2Out-pimOut)<<" "; //15,16 ok
  output<<~(beamOut+target-kp1Out-kp2Out)<<" "; //17
  output<<~(beamOut+target-kp1Out)<<" "<<~(beamOut+target-kp2Out)<<" "<<~(beamOut+target-pimOut)<<" "<<~(pimOut+kp1Out)<<" "<<~(pimOut+kp2Out)<<" "; //18-22  ok
  double mppim = -10;
  fourVec proton;
...
//get the meson system 4-vector
 ...
 // Now rotate to the helicity frame of X so the z axis is along the X momentum  
  ...
  kp1_1 *=L1;
  kp2_1 *=L1;
  output<<kp2_1.cosTheta()<<" "<<kp2_1.phi()/M_PI<<" ";  // 107 108  ok
 ...
 // Now rotate to the helicity frame of Y2 so the z axis is along the X momentum  
  ...
  kp1_2 *=L2;
  kp2_2 *=L2;
  output<<kp1_2.cosTheta()<<" "<<kp1_2.phi()/M_PI<<" "; // 109 110   ok
...
//now we are in the X (cascade*) helicity rest frame , output the 2-body decay angles of Pi- in X rest frame
  ...
  if(fabs(ctpim)>0&&fabs(phipim)>0){
    ...
    output<<ctpim<<" "<<phipim<<" ";  //111,112  ok
  }
  else{
    output<<" -10 "<<" "<<" -10 "<<" ";         //same as above
  }
...
///JB\\ pim_1 is in the X resframe with z aligned with beam, and cross is in cm with z aligned with beam	  
  output<<(Cross - pim_1.V()).cosTheta()<<endl;   //113
