...
Feb 22 2012:  trying to add kinfit
...
//jb v15_kinfit
cout << ++nlab << "\t(first try with kinfit){kfprob}"<<endl;
cout << ++nlab << "\t(first try with kinfit){kfchi2}"<<endl;
...
int ProcessEvent(clasEvent &my_event,int verbose, int IFLAG)
{
 ...
 evtno=my_event.event();
  ...
  fourVec lab_kp1,lab_kp2, lab_pim;  //jb v15_kf2
  lab_kp1 = kp1;
  lab_kp2 = kp2;
  lab_pim = pim;
...
//--------------jb v15_kinfit----------------------------------------
vector<clasParticle> cp_kpkppim {lab_kp1, lab_kp2, lab_pim};
