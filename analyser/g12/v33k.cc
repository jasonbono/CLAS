...
void printLabels()
{
 ...
 cout << ++nlab << "\t(kfit)(Cascadem - pim){kf_mcasmMpim}"<<endl;
 ...
 // kf2                     for the higher cascade resonance
...
/*
//-------------------------------------------not workiing----------------------KF2---------------------------------------------------------------
 vector<clasParticle> cp_kpkppim2;
 cp_kpkppim2.push_back(cpkp1);
 cp_kpkppim2.push_back(cpkp2);
 cp_kpkppim2.push_back(cppim);
 kinfit_output_t kfit_kpkppim2 = kinematic_fit(beam, PROTON_MASS, cp_kpkppim2, CASCADE0_MASS, "g12", Vert);

 //fill parameters with new fitted values
 fourVec kf2_kp1 = kfit_kpkppim2.parts[0];
 fourVec kf2_kp2 = kfit_kpkppim2.parts[1];
 fourVec kf2_pim = kfit_kpkppim2.parts[2];
 fourVec kf2_beam = kfit_kpkppim2.beam;

 //--------- make the pdg lambda and cascade---------------------------------
 //the missing 4-momentums
 fourVec kf2_miss = kf2_beam + target - kf2_kp1 - kf2_kp2 - kf2_pim;
 fourVec kf2_miss2 = kf2_beam + target - kf2_kp1 - kf2_kp2;
 //the pdg cascade0 energy
 double kf2_Ecascade0 = sqrt(CASCADE0_MASS*CASCADE0_MASS + kf_miss.r()*kf_miss.r());
 // fill the cascade0
 fourVec kf_cascade0;
 kf_cascade0.set(kf2_Ecascade0, threeVec(kf2_miss.x(), kf2_miss.y(), kf2_miss.z()));
 //-----------------------------------------------------------------------------


 // prob, chi2 and pulls
 double kf2_prob = kfit_kpkppim2.prob;
 double kf2_chi2 = kfit_kpkppim2.chi2;
 double kf2_pull_beam = kfit_kpkppim2.pull_beam;
