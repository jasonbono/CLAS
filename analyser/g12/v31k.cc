...
void printLabels()
{
 ...
 cout << ++nlab << "\t(kfit2)(lambda + pim){kf2_mlampim}"<<endl;  
 cout << ++nlab << "\t(kfit2)(Cascadem - pim){kf2_mcasmMpim}"<<endl;  
 ...
 cout << ++nlab << "\t(pi) lambda angle x{COSth_lam_x}"<<endl; 
 cout << ++nlab << "\t(pi) lambda angle y{COSth_lam_y}"<<endl; 
 cout << ++nlab << "\t(pi) lambda angle z{COSth_lam_z}"<<endl;
 ...
 */ 
 ...
 //-----------------------------------------------------------------KF2---------------------------------------------------------------
 vector<clasParticle> cp_kpkp;
 cp_kpkp.push_back(cpkp1);
 cp_kpkp.push_back(cpkp2);
 kinfit_output_t kfit_kpkp = kinematic_fit(beam, PROTON_MASS, cp_kpkp, CASCADE_MASS, "g12", Vert);

 //fill parameters with new fitted values
 fourVec kf2_kp1 = kfit_kpkppim.parts[0];
 fourVec kf2_kp2 = kfit_kpkppim.parts[1];
 fourVec kf2_beam = kfit_kpkppim.beam;

 //--------- make the pdg lambda and cascade---------------------------------
 //the missing 4-momentums
 fourVec kf2_miss = kf2_beam + target - kf2_kp1 - kf2_kp2 - pim;
 fourVec kf2_miss2 = kf2_beam + target - kf2_kp1 - kf2_kp2;
 //the pdg energy
 double kf2_Elambda = sqrt(LAMBDA_MASS*LAMBDA_MASS + kf2_miss.r()*kf2_miss.r());
 double kf2_Ecascadem = sqrt(CASCADE_MASS*CASCADE_MASS + kf2_miss2.r()*kf2_miss2.r());
 // fill the lambda and cascade
 fourVec kf2_lambda, kf2_cascadem;
 kf2_lambda.set(kf2_Elambda, threeVec(kf2_miss.x(), kf2_miss.y(), kf2_miss.z()));
 kf2_cascadem.set(kf2_Ecascadem, threeVec(kf2_miss2.x(), kf2_miss2.y(), kf2_miss2.z()));
 //-----------------------------------------------------------------------------


 // prob, chi2 and pulls
 double kf2_prob = kfit_kpkp.prob;
 double kf2_chi2 = kfit_kpkp.chi2;
 double kf2_pull_beam = kfit_kpkp.pull_beam;
 ...
 output << ~(target + kf2_beam - kf2_kp1 - kf2_kp2 - pim) << " ";
 output << ~(target + kf2_beam - kf2_kp1 - kf2_kp2) << " ";
 output << ~(kf2_lambda + pim) << " ";
 output << ~(kf2_cascadem - pim) << " ";
