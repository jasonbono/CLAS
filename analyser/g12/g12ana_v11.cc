...
October 23 2011 v07  to
 
 
 
 
 ok out the kinematic fiting and the momentum correction.  took out 10 branches for kinematic fiting so there should be 121-10=111
...
//jbv07_3 the cascade and lambda missing mass square without beam energy corrections 
 cout << ++nlab << "\tmissing mass square without beam energy corrections of (kp1+kp2){wo_mm2kpkp}"<<endl;
 cout << ++nlab << "\tmissing mass square without beam energy corrections of (kp1+kp2+pim){wo_mm2kpkppim}"<<endl;
...
int ProcessEvent(clasEvent &my_event,int verbose, int IFLAG)
{
  ...
  fourVec wo_beam;  //jbv07_3  made for showing missing mass without beam energy corrections.
  double wo_beamp = -100.0;   //jbv07_3  made for showing missing mass without beam energy corrections.
 ...
 evtno=my_event.event();
  ...
  wo_beam.set(wo_beamp,threeVec(0.0,0.0,wo_beamp)); //jbv07_3  made for showing missing mass without beam energy corrections.
...
//jb output << (psbeam + target - kp1 - kp2-pim).lenSq() << " ";
  output << (wo_beam + target - kp1 - kp2).lenSq() << " ";   //jbv07_3
  output << (wo_beam + target - kp1 - kp2-pim).lenSq() << " ";  //jbv07_3 
...
/* Exit from function if missing the requisite banks... */
  ...
  for (i=0;i<TBID->bank.nrow;i++){
    ...
    if(trk_ind){     
      tprop=(TBTR->tbtr[trk_ind-1].vert.z - g12targz)/SPEED_OF_LIGHT*1e9;  //jb v11 i think i need to take this out
