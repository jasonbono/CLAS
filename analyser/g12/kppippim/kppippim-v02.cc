...
void printLabels(){
 ...
 cout << ++nlab << "\tMass(mmk0_no_ecor){mmk0_no_ecor}"<<endl;
 cout << ++nlab << "\tMass(mmkpk0_no_ecor){mmkpk0_no_ecor}"<<endl;
 ...
 cout << ++nlab << "\tMass(mmk0_no_pcor){mmk0_no_pcor}"<<endl;
 cout << ++nlab << "\tMass(mmkpk0_no_pcor){mmkpk0_no_pcor}"<<endl;
 ...
 cout << ++nlab << "\tMass(mmk0_no_eloss){mmk0_no_eloss}"<<endl;
 cout << ++nlab << "\tMass(mmkpk0_no_eloss){mmkpk0_no_eloss}"<<endl; 
 ...
 cout << ++nlab << "\tMass(mmk0_no_corrections){mmk0_no_corrections}"<<endl;
 cout << ++nlab << "\tMass(mmkpk0_no_corrections){mmkpk0_no_corrections}"<<endl;
...
int ProcessEvent(clasEvent &my_event,int verbose, int IFLAG)
{
 ...
 //---particle stuff----
 fourVec kp,pip, pim, pipi, k0;
 fourVec kp_no_eloss, pip_no_eloss, pim_no_eloss, pipi_no_eloss, k0_no_eloss;
 fourVec kp_no_pcor, pip_no_pcor, pim_no_pcor, pipi_no_pcor, k0_no_pcor; 
 fourVec kp_no_corrections, pip_no_corrections, pim_no_corrections, pipi_no_corrections, k0_no_corrections;
 ...
 double Ek0_no_corrections = sqrt(K0_MASS*K0_MASS + pipi_no_corrections.r()*pipi_no_corrections.r());
 k0_no_corrections.set(Ek0_no_corrections, threeVec(k0_no_corrections.x(), k0_no_corrections.y(), k0_no_corrections.z()));
 
 kp_no_eloss = kp_no_corrections;
 pip_no_eloss = pip_no_corrections;
 pim_no_eloss = pim_no_corrections;
 pipi_no_eloss = pipi_no_corrections;
 k0_no_eloss = k0_no_corrections;
...
//--------------------------------------------------beam energy corections,eloss,kaon switching --------------------------------------------------
 ...
 pip_no_pcor = pip;
 pim_no_pcor = pim;
 pipi_no_pcor = pip + pim;
 double Ek0_no_pcor = sqrt(K0_MASS*K0_MASS + pipi_no_pcor.r()*pipi_no_pcor.r());
 k0_no_pcor.set(Ek0_no_pcor, threeVec(k0_no_pcor.x(), k0_no_pcor.y(), k0_no_pcor.z()));
 ...
 float mmkpk0 =  ~(beam + target - kp - k0);
 float mmk0 =  ~(beam + target - k0);
 threeVec Vert = my_event.V();

 
 int st=0;
 if(abs(my_event.scVtime()-my_event.stVtime())<=1) st=1; 
 
 int bb=0;
 if(beamp > 2.5) bb=1;
 
 int mm=0;
 if(mmkppippim > 0 && mmpippim > 0 && mmk0 > 0 && mmkpk0 > 0 ) mm=1;
 ...
 if(st&&bb&&mm)
 {
  ...
  output << ~(beam_no_ecor + target - k0) <<" ";
  output << ~(beam_no_ecor + target - kp - k0 ) <<" ";
  ...
  output << ~(beam + target - k0_no_pcor) <<" ";
  output << ~(beam + target - kp_no_pcor - k0_no_pcor ) <<" ";
  ...
  output << ~(beam + target - k0_no_eloss) <<" ";
  output << ~(beam + target - kp_no_eloss - k0_no_eloss ) <<" " ;
  ...
  output << ~(beam_no_ecor + target - k0_no_corrections) <<" ";
  output << ~(beam_no_ecor + target - kp_no_corrections - k0_no_corrections ) <<" ";
