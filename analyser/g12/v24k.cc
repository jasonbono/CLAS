...
#include "kinfit.hpp" //jb v15_kinfit
...
//jb_v23k 
// g12 kinematic fitting variables
...
//JB\\ cout << ++nlab << "\tmissing mass square of (kp+kp+pim)using ps beam{psmm2kpkppim}"<<endl;
 ...
 //missing masses
  ...
  cout << ++nlab << "\tMissing Mass(pip2){mmpip2}"<<endl;
	...
	//jb v24k
	cout << ++nlab << "\t(pi) pion angle x{COSth_pi_x}"<<endl; 
	cout << ++nlab << "\t(pi) pion angle y{COSth_pi_y}"<<endl; 
	cout << ++nlab << "\t(pi) pion angle z{COSth_pi_z}"<<endl;
 ...
 //X rest frame angles
  ...
  cout << ++nlab << "\t(pi) pi norm angle1{PIangle_ANA1}"<<endl; //jb v18
  cout << ++nlab << "\t(pi) pi norm angle2{PIangle_ANA2}"<<endl; //jb v20
  cout << ++nlab << "\t(pi) pi norm angle3{PIangle_ANA3}"<<endl; //jb v21
  ...
  //jb v21
  cout << ++nlab << "\t(tests) {casmrestE}}"<<endl;
  cout << ++nlab << "\t(tests) {casmrestM}}"<<endl;
  cout << ++nlab << "\t(tests) {casmrestX}}"<<endl;
  cout << ++nlab << "\t(tests) {casmrestY}}"<<endl;
  cout << ++nlab << "\t(tests) {casmrestZ}}"<<endl;
  cout << ++nlab << "\t(tests) {EpiPLUSlambda}}"<<endl;
  //cout << ++nlab << "\t(tests) {XrestPX}}"<<endl;
  //cout << ++nlab << "\t(tests) {XrestPY}}"<<endl;
  //cout << ++nlab << "\t(tests) {XrestPZ}}"<<endl;

  //jb v18	
  cout << ++nlab << "\t(tests) {t3x}}"<<endl;
  cout << ++nlab << "\t(tests) {t3y}}"<<endl;
  cout << ++nlab << "\t(tests) {t3z}}"<<endl;
  cout << ++nlab << "\t(tests) {t3r}}"<<endl;
  cout << ++nlab << "\t(tests) {t4x}}"<<endl;
  cout << ++nlab << "\t(tests) {t4y}}"<<endl;
  cout << ++nlab << "\t(tests) {t4z}}"<<endl;
  cout << ++nlab << "\t(tests) {t4r}}"<<endl;
...
kinfit_output_t kfit_kpkppim = kinematic_fit(beam, PROTON_MASS, cp_kpkppim, LAMBDA_MASS, "g12", Vert);
...
cascadem_my = cascadem;
  ...
  fourVec KP1, KP2, PIM, BEAM, CAS, LAM; 
  KP1 = kp1;
  KP2 = kp2;
  PIM = pim;
  BEAM = beam;
  ...
  //now the quanties will all be boosted to the cascade restframe frame
  lorentzTransform LCAS;
  LCAS.set(beam + target);
  ...
  // obtain the unit threevector coordinate system and unit pion momentum vector
  threeVec TcasNorm, TpiNorm;
  threeVec xx,yy,zz;
  TcasNorm.set(CAS.x()/CAS.r(), CAS.y()/CAS.r(), CAS.z()/CAS.r() );
  TpiNorm.set(PIM.x()/PIM.r(), PIM.y()/PIM.r(), PIM.z()/PIM.r() );
  zz.set( BEAM.x()/BEAM.r(), BEAM.y()/BEAM.r(), BEAM.z()/BEAM.r() );
  yy = zz/TcasNorm;
  xx = yy/zz;
  
  //now compute the angles of the pion
  double COSth_pi_x, COSth_pi_y, COSth_pi_z;
  COSth_pi_x = TpiNorm.x() * xx.x() + TpiNorm.y() * xx.y() + TpiNorm.z() * xx.z();
  COSth_pi_y = TpiNorm.x() * yy.x() + TpiNorm.y() * yy.y() + TpiNorm.z() * yy.z();
  COSth_pi_z = TpiNorm.x() * zz.x() + TpiNorm.y() * zz.y() + TpiNorm.z() * zz.z();
...
//jb v18 now everything i need is in the CM frame with the beam aligned on the zaxis:  here i can define a the normal to the production plane
  threeVec Cross_1;
  Cross_1 = beam/X_1;
  //jb v21 
  threeVec Cross_my;
  Cross_my = beam/cascadem_my;



//jb v18 now to boost the pion and the lambda to the X_1 reference frame:
//jb v21  lorentzTransform L9;  
//jb v21  L9.set(X_1);
//jb v21  pim_1 *= L9;
//jb v21  lambda_1 *= L9;

  //jb v21
  lorentzTransform Lmy;  
  Lmy.set(cascadem_my);
  pim_1 *= Lmy;
  lambda_1 *= Lmy;

//jb v18 now adding the lab angles
  threeVec xlab, ylab, zlab;
...
/* 
 //jb v18 this block was taken out 
...
// theese below are the cosine of the angles of intrest.
  ...
  //jb v18 this should be the same as above
  output<< (Cross.x() * pim_1.x() + Cross.y() * pim_1.y() + Cross.z() * pim_1.z()) / (sqrt(pim_1.x() * pim_1.x() + pim_1.y() * pim_1.y() + pim_1.z() * pim_1.z()) * sqrt(Cross.x() * Cross.x() + Cross.y() * Cross.y() + Cross.z() * Cross.z())) <<" "; //jb v18
  //jb v20 so should this one
  output<< (Cross_1.x() * pim_1.x() + Cross_1.y() * pim_1.y() + Cross_1.z() * pim_1.z()) / (Cross_1.r() * pim_1.r()) <<" ";
  //jb v21 and this one
  output<< (Cross_my.x() * pim_1.x() + Cross_my.y() * pim_1.y() + Cross_my.z() * pim_1.z()) / (Cross_my.r() * pim_1.r()) <<" ";
  ...
  //jb v21 testing to make sure that Xi is accualy at rest from the boose L9
  cascadem_my *= Lmy;
  output << cascadem_my.t() << " ";
  output << ~cascadem_my << " ";
  output << cascadem_my.x() << " ";
  output << cascadem_my.y() << " ";
  output << cascadem_my.z() << " ";
  output << (pim_1 + lambda_1).t() << " ";


  //jb v18 doing some tests
  threeVec test3;
  test3.set(10.0, 20.0, 30.0);
  fourVec test4;
  test4.set(1.0, 2.0, 3.0, 4.0);
