/*
November 7 2012: Added ecor and pcor.
...
int ProcessEvent(clasEvent &my_event,int verbose, int IFLAG)
{
 ...
 fourVec beam,fixbeam,target(PROTON_MASS,threeVec(0.0,0.0,0.0));
 double beamp_woecor = -100, beamp = -100.0, fixbeamp = -100.0;
...
//--------------------------------------------------beam energy corections,eloss,kaon switching --------------------------------------------------
 ...
 if(corr)
 {
  ...
  {
   beamp = clas::g12::corrected_beam_energy(my_event.run(), beamp_woecor);
  }
 }


 beam.set(beamp,threeVec(0.0,0.0,beamp));  
 kp1=my_event.cp(KPlus,1).p(); //11
 kp2=my_event.cp(KPlus,2).p();
 pim=my_event.cp(PiMinus,1).p(); //9
 ...
 if(corr) 
 {
  ...
  //pim
  double id_pim = 11; // kaon (geant 3 ID codes)
...
// if((Vert.x()*Vert.x()+Vert.y()*Vert.y())<=4) targXY=1;  //\\ this too
 ...
 if(st&&bb)  //jb v35k putting the filter back
 {
  ...
  output << ~(beam_woecor + target-kp1-kp2 - pim) << " ";
  output << ~(beam + target - kp1_woeloss - kp2_woeloss) << " ";
  output << ~(beam + target-kp1_woeloss-kp2_woeloss - pim_woeloss)<< " ";
  output << ~(beam_woecor + target - kp1_woeloss - kp2_woeloss) << " "; 
  output << ~(beam_woecor + target-kp1_woeloss-kp2_woeloss - pim_woeloss)<< " ";
