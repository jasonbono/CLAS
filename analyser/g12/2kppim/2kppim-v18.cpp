/*
April 16 2014: v18
1) added phi which is the result of an if statment deciding which kaon to use.
2) added "cosangle" 

April 11 2014: v17
1) fixed the lable interchange
2) distinguished between slowphi and fastphi

April 10 2014: v16
1) added MESON helicity "phi" and "theta"
...
//helicity
 cout << ++nlab << "\t(hel) helicty{fasttheta}"<<endl;
 cout << ++nlab << "\t(hel) helicty{fastphi}"<<endl;
 cout << ++nlab << "\t(hel) helicty{slowtheta}"<<endl;
 cout << ++nlab << "\t(hel) helicty{slowphi}"<<endl;
 cout << ++nlab << "\t(hel) helicty{cosangle}"<<endl;
 cout << ++nlab << "\t(hel) helicty{phi}"<<endl;
...
*/
  ...
  //=====take steps to calculate theta and phi of the kp1 in the X helicity rest frame==========
  fourVec Xmes = kp1 + kp2;
  fourVec Xmes_cm = Xmes;
  fourVec kp1_h = kp1;      // _h for helicity
  fourVec kp2_h = kp2; 
  fourVec beam_h = beam;
  //cm boost
  lorentzTransform L;
  ...
  kp2_h *= L;
  Xmes_cm *= L;
  //boost to Xmes (meson Xmes!) rest frame as defined in the center of mass
  L.set(Xmes_cm);
  kp1_h *= L;
  kp2_h *= L;
  beam_h *= L;
  //align z-aXmesis with the Xmes momentum (Xmes momentum in the center of mass)
  L.set(Xmes_cm.phi(),Xmes_cm.theta(),0.0);
  beam_h *= L;
  kp1_h *= L;
  //align y-axis along "cross" which defines the beam/Xmes production plane (beam_h and Xmes_cm) 
  threeVec cross;
  cross = beam_h/Xmes_cm;
  L.set(cross.phi(),cross.theta()-M_PI/2.0,-M_PI/2.0);
  kp1_h *= L;
  //output costheta and phi of the k_fast
  output<<kp1_h.cosTheta()<<" "<<kp1_h.phi()<<" ";
  output<<kp2_h.cosTheta()<<" "<<kp2_h.phi()<<" ";

  //pick the "positve" meson
  float cosangle = kp1_h.x()*cross.x() + kp1_h.y()*cross.y() + kp1_h.z()*cross.z();
  if (cosangle >= 0 ){
	output<<cosangle<<" "<<kp1_h.phi()<<" ";
  }
  else{
	output<<cosangle<<" "<<kp2_h.phi()<<" ";
  }
