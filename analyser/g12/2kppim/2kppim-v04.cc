...
int ProcessEvent(clasEvent &my_event,int verbose, int IFLAG)
{
 ...
 //kp1_no_pcor = kp1_no_corrections;
...
//--------------------------------------------------beam energy corections,eloss,kaon switching --------------------------------------------------
 ...
 //setup the eloss corrected four vectors
 ...
 if(corr && datatype) 
 {
  ...
  ///--------------kp1,kp2,pim------------------
  //kp1
  double id_kp1 = 11; // kaon (geant 3 ID codes)
  double p_kp1 = kp1.r(); // total momentum
  double px_kp1 = kp1.x();
  double py_kp1 = kp1.y();
 
  double pz_kp1 = sqrt(p_kp1*p_kp1 - px_kp1*px_kp1 - py_kp1*py_kp1);
  double phi_kp1 = atan2(py_kp1,px_kp1);
  double theta_kp1 = acos(pz_kp1/p_kp1);

  double new_p_kp1 = p_kp1 + pcor.pcor(phi_kp1,id_kp1);

  double new_px_kp1 = (new_p_kp1 / p_kp1) * px_kp1;
  double new_py_kp1 = (new_p_kp1 / p_kp1) * py_kp1;
  double new_pz_kp1 = (new_p_kp1 / p_kp1) * pz_kp1;

  kp1.set( sqrt(KP_MASS*KP_MASS + new_px_kp1*new_px_kp1), new_px_kp1, new_py_kp1, new_pz_kp1);


  //kp2
  double id_kp2 = 11; // kaon (geant 3 ID codes)
  double p_kp2 = kp2.r(); // total momentum
  double px_kp2 = kp2.x();
  double py_kp2 = kp2.y();

  double pz_kp2 = sqrt(p_kp2*p_kp2 - px_kp2*px_kp2 - py_kp2*py_kp2);
  double phi_kp2 = atan2(py_kp2,px_kp2);
  double theta_kp2 = acos(pz_kp2/p_kp2);

  double new_p_kp2 = p_kp2 + pcor.pcor(phi_kp2,id_kp2);

  double new_px_kp2 = (new_p_kp2 / p_kp2) * px_kp2;
  double new_py_kp2 = (new_p_kp2 / p_kp2) * py_kp2;
  double new_pz_kp2 = (new_p_kp2 / p_kp2) * pz_kp2;

  kp2.set( sqrt(KP_MASS*KP_MASS + new_px_kp2*new_px_kp2), new_px_kp2, new_py_kp2, new_pz_kp2);



  //pim
  double id_pim = 9; // kaon (geant 3 ID codes)
  double p_pim = pim.r(); // total momentum
  double px_pim = pim.x();
  double py_pim = pim.y();

  double pz_pim = sqrt(p_pim*p_pim - px_pim*px_pim - py_pim*py_pim);
  double phi_pim = atan2(py_pim,px_pim);
  double theta_pim = acos(pz_pim/p_pim);

  double new_p_pim = p_pim + pcor.pcor(phi_pim,id_pim);

  double new_px_pim = (new_p_pim / p_pim) * px_pim;
  double new_py_pim = (new_p_pim / p_pim) * py_pim;
  double new_pz_pim = (new_p_pim / p_pim) * pz_pim;

  pim.set( sqrt(PI_CHARGED_MASS*PI_CHARGED_MASS + new_px_pim*new_px_pim), new_px_pim, new_py_pim, new_pz_pim);



  ///--------------kp1_no_eloss,kp2_no_eloss,pim_no_eloss------------------
  //kp1_no_eloss
  double id_kp1_no_eloss = 11; // kaon (geant 3 ID codes)
  double p_kp1_no_eloss = kp1_no_eloss.r(); // total momentum
  double px_kp1_no_eloss = kp1_no_eloss.x();
  double py_kp1_no_eloss = kp1_no_eloss.y();
 
  double pz_kp1_no_eloss = sqrt(p_kp1_no_eloss*p_kp1_no_eloss - px_kp1_no_eloss*px_kp1_no_eloss - py_kp1_no_eloss*py_kp1_no_eloss);
  double phi_kp1_no_eloss = atan2(py_kp1_no_eloss,px_kp1_no_eloss);
  double theta_kp1_no_eloss = acos(pz_kp1_no_eloss/p_kp1_no_eloss);

  double new_p_kp1_no_eloss = p_kp1_no_eloss + pcor.pcor(phi_kp1_no_eloss,id_kp1_no_eloss);

  double new_px_kp1_no_eloss = (new_p_kp1_no_eloss / p_kp1_no_eloss) * px_kp1_no_eloss;
  double new_py_kp1_no_eloss = (new_p_kp1_no_eloss / p_kp1_no_eloss) * py_kp1_no_eloss;
  double new_pz_kp1_no_eloss = (new_p_kp1_no_eloss / p_kp1_no_eloss) * pz_kp1_no_eloss;

  kp1_no_eloss.set( sqrt(KP_MASS*KP_MASS + new_px_kp1_no_eloss*new_px_kp1_no_eloss), new_px_kp1_no_eloss, new_py_kp1_no_eloss, new_pz_kp1_no_eloss);


  //kp2_no_eloss
  double id_kp2_no_eloss = 11; // kaon (geant 3 ID codes)
  double p_kp2_no_eloss = kp2_no_eloss.r(); // total momentum
  double px_kp2_no_eloss = kp2_no_eloss.x();
  double py_kp2_no_eloss = kp2_no_eloss.y();

  double pz_kp2_no_eloss = sqrt(p_kp2_no_eloss*p_kp2_no_eloss - px_kp2_no_eloss*px_kp2_no_eloss - py_kp2_no_eloss*py_kp2_no_eloss);
  double phi_kp2_no_eloss = atan2(py_kp2_no_eloss,px_kp2_no_eloss);
  double theta_kp2_no_eloss = acos(pz_kp2_no_eloss/p_kp2_no_eloss);

  double new_p_kp2_no_eloss = p_kp2_no_eloss + pcor.pcor(phi_kp2_no_eloss,id_kp2_no_eloss);

  double new_px_kp2_no_eloss = (new_p_kp2_no_eloss / p_kp2_no_eloss) * px_kp2_no_eloss;
  double new_py_kp2_no_eloss = (new_p_kp2_no_eloss / p_kp2_no_eloss) * py_kp2_no_eloss;
  double new_pz_kp2_no_eloss = (new_p_kp2_no_eloss / p_kp2_no_eloss) * pz_kp2_no_eloss;

  kp2_no_eloss.set( sqrt(KP_MASS*KP_MASS + new_px_kp2_no_eloss*new_px_kp2_no_eloss), new_px_kp2_no_eloss, new_py_kp2_no_eloss, new_pz_kp2_no_eloss);


 
  //pim_no_eloss
  double id_pim_no_eloss = 9; // kaon (geant 3 ID codes)
  double p_pim_no_eloss = pim_no_eloss.r(); // total momentum
  double px_pim_no_eloss = pim_no_eloss.x();
  double py_pim_no_eloss = pim_no_eloss.y();

  double pz_pim_no_eloss = sqrt(p_pim_no_eloss*p_pim_no_eloss - px_pim_no_eloss*px_pim_no_eloss - py_pim_no_eloss*py_pim_no_eloss);
  double phi_pim_no_eloss = atan2(py_pim_no_eloss,px_pim_no_eloss);
  double theta_pim_no_eloss = acos(pz_pim_no_eloss/p_pim_no_eloss);

  double new_p_pim_no_eloss = p_pim_no_eloss + pcor.pcor(phi_pim_no_eloss,id_pim_no_eloss);

  double new_px_pim_no_eloss = (new_p_pim_no_eloss / p_pim_no_eloss) * px_pim_no_eloss;
  double new_py_pim_no_eloss = (new_p_pim_no_eloss / p_pim_no_eloss) * py_pim_no_eloss;
  double new_pz_pim_no_eloss = (new_p_pim_no_eloss / p_pim_no_eloss) * pz_pim_no_eloss;

  pim_no_eloss.set( sqrt(PI_CHARGED_MASS*PI_CHARGED_MASS + new_px_pim_no_eloss*new_px_pim_no_eloss), new_px_pim_no_eloss, new_py_pim_no_eloss, new_pz_pim_no_eloss);
...
// if((Vert.x()*Vert.x()+Vert.y()*Vert.y())<=4) targXY=1;  //\\ this too
 ...
 if(st&&bb)  //jb v35k putting the filter back
 {
  ...
  //--------------------------------------------BOOST TOPOLOGY TO Xi (gotfried-jackson) FRAME-----------------------------------------------
  ...
  // fill unit three vectors of beam,kpkp,pim,lam
  ...
  //unit coordinate vectors (z' = beam momentum) , (y' = beam.cross.kpkp), (x' = z'.cross.y') 
  threeVec xx,yy,zz;  
