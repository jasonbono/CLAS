...
//=====take steps to calculate theta and phi of the pip in the X helicity rest frame==========
  ...
  pip_1 *= L;
  X *= L;
  //define the X production plane
  threeVec Cross;
  Cross = beam/X;  //should i use p/beam? -> no because this is before the boost, if it were after than yes becasue X would have zero beta thus the production plane def would be unstable
  //align with y-axis
  L.set(Cross.phi(),Cross.theta()-M_PI/2.0,-M_PI/2.0);
  pip_1 *= L;
  X *= L;
  //rotate z-axis to be along X momentum (X-helicity-frame orentation)
  L.set(X.phi(),X.theta(),0.0);
  pip_1 *= L;
  X *= L;
  //finally boost to X rest frame  (this will not change the direction of y since it is normal)
  L.set(X);
  pip_1 *= L;
  output<<pip_1.cosTheta()<<" "<<pip_1.phi()<<" ";
