...
open(ANATEMPLATE, "$dir/analyze-template.sh")|| die "Can't open analyze-template.sh $!\n"; 
open(ANASCRIPT, ">$dir/$newdir/analyze-script_${topology}-${version}.sh") || die "Can't open analyze-script_${topology}-${version}.sh $!\n";

open(JOBTEMPLATE, "$dir/job-template.sh")|| die "Can't open job-template.sh $!\n"; 
open(JOBSCRIPT, ">$dir/$newdir/job-script_${topology}-${version}.sh") || die "Can't open job-script_${topology}-${version}.sh $!\n";
...
system ("chmod u+x $dir/$newdir/job-script_${topology}-${version}.sh");
system ("chmod u+x $dir/$newdir/analyze-script_${topology}-${version}.sh");
