...
{	
	...
	*/
	...
	Float_t gj_COS_pi_x,gj_COS_pi_y,gj_COS_pi_z;
	Float_t KPKPcmtheta;
//	Float_t mmkp1;
	...
	tree->Branch("gj_COS_pi_x",&gj_COS_pi_x,"gj_COS_pi_x/F");
	tree->Branch("gj_COS_pi_y",&gj_COS_pi_y,"gj_COS_pi_y/F");
	tree->Branch("gj_COS_pi_z",&gj_COS_pi_z,"gj_COS_pi_z/F");
	...
	{	
		...
		//define cooridinate system in cm frame
		TVector3 unitZ_cm;
		TVector3 unitY_cm;
		TVector3 unitX_cm;
		...
		//pion polarization angles in cm frame
		...
		TVector3 unitY_xi;
		TVector3 unitX_xi;
		unitZ_xi = (tbeam_xi).Unit();  //beam
		unitY_xi = tbeam_xi.Cross(tkpkp_xi).Unit(); //beam.x.kpkp
		unitX_xi = unitZ_xi.Cross(unitY_xi).Unit(); //Z.x.Y
		//pion angles in jackson frame
		gj_COS_pi_x = cos(unitX_xi.Angle(tpi_xi)); 
		gj_COS_pi_y = cos(unitY_xi.Angle(tpi_xi));
		gj_COS_pi_z = cos(unitZ_xi.Angle(tpi_xi));
		...
		if(count % 1000==0)
  		{
			...
			//cout << "Ecas = " << Ecas <<endl;
			//cout << endl;
			//cout << "checking X " << unitX_xi.Angle(unitX_xi) << "  " << unitX_xi.Mag() << " " << unitX_xi.Angle(unitY_xi) <<endl;
			// cout << " check the coordinate system" <<endl;
			// cout << " lab vs jackson coordinates: " << cos(unitX_xi.Angle(unitX_lab)) << " " <<  cos(unitY_xi.Angle(unitY_lab)) << " " <<  cos(unitZ_xi.Angle(unitZ_lab)) << endl;
			// cout << " jackson coordinates:  " << cos(unitX_xi.Angle(unitY_xi)) << " " <<  cos(unitX_xi.Angle(unitZ_xi)) << " " <<  cos(unitY_xi.Angle(unitZ_xi)) << endl;
			// cout << " lab coordinates:  " << cos(unitX_lab.Angle(unitY_lab)) << " " <<  cos(unitX_lab.Angle(unitZ_lab)) << " " <<  cos(unitY_lab.Angle(unitZ_lab)) << endl;
			// cout << " magnitudes:  " << unitX_lab.Mag() << unitY_lab.Mag() << unitZ_lab.Mag() << unitX_xi.Mag() << unitY_xi.Mag() << unitZ_lab.Mag() <<endl;
			// cout << " " <<endl;
