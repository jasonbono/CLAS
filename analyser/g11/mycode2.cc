//edited on aug 30 by JB.  to find places of editing look for ///JB///  
// added on thing to lable file and printed it out in the process event
...
void printLabels()
{
 ...
 cout << ++nlab << "\tbeam energy from ps tagger correction{psbeam}"<<endl;
 ...
 cout << ++nlab << "\tt energy transfer(kp2+pim - beam){tkp2pim}"<<endl;
  ...
  cout << ++nlab << "\tmissing mass square of using ps beam (K+K+) {psmm2kpkp}"<<endl;
 ...
 cout << ++nlab << "\tmissing mass square of (kp+kp+pim)using ps beam{psmm2kpkppim}"<<endl;
 ...
 //missing masses
  ...
  cout << ++nlab << "\tmiss momentum{misse}"<<endl; 
   ...
   //constrained masses
    ...
    cout << ++nlab << "\tMass(Cascade, pim) ps method{mpscaspim}"<<endl;
...
cout << ++nlab << "\t(K+2) lab angle{kp2labtheta}"<<endl;
 ...
 //X rest frame angles
  ...
  cout << ++nlab << "\t(pi) Xi norm angle{PIangle}"<<endl;
  }
...
//now plot the masses
  ...
  output << (psbeam + target - kp1 - kp2-pim).lenSq() << " ";  
  ...
  output<<~(cascade + kp1) <<" "<<~(cascade + kp2)<<" "<<~(cascade + pim)<<" "<<~(lambda+pim)<<" "<<~(pscascade + pim)<<" "<<~(fixbeam+target-kp1-kp2)<<" "<<fixbeamp<<" ";
...
///JB///////////////// NOW MAKE pim_1=pim AND BOOST TO X REST FRAME        JB 
	  fourVec pim_1;
	  lorentzTransform L9;  
	  L9.set(X);
 ...
 // Now rotate to the helicity frame of X so the z axis is along the X momentum 
  ...
  pim *=L;
	  ...
	  ///JB/////////////// edited the 2 print statements: removed <<endl; and but <<""; in both  
...
///JB///////////////////////// NOW GET THE ANGLE BETWEEN pim_1 and normal and print it out	  
