...
TFile *fGen = TFile::Open("/Users/Bono/clas/rootfiles/g12/mc/i10/SendHome/MC-i10t3-t3-gamproot.root");
...
void PasyAC()
{
	...
	for (int b = 0; b < Nb; b++)
	{
		for (int c = 0; c < Nc; c++)
		{
			...
			hMC[b][c] = (TH1F*)h->Clone(Form("hMC%i%i",b,c));
			hGen[b][c] = (TH1F*)h->Clone(Form("hGen%i%i",b,c));
			hAC[b][c] = (TH1F*)h->Clone(Form("hAC%i%i",b,c));
			tMC->Draw(Form("COS_pi_y>>hMC%i%i",b,c),EXibin[b][c]);
			tGen->Draw(Form("COS_pi_y>>hGen%i%i",b,c),EXibin[b][c]);
			hAC[b][c]->Divide(hMC[b][c],hGen[b][c]);
			
			
			float asytot = 0;
			float negtot = 0;
			float postot = 0;
			hDataN[b][c] = (TH1F*)h->Clone(Form("h%i%iN",b,c));
			hCorN[b][c] = (TH1F*)h->Clone(Form("hCor%i%iN",b,c));
			hDataN[b][c]->SetFillColor(46);		
			hCorN[b][c]->SetFillColor(46);	
			tData->Draw(Form("COS_pi_y>>h%i%iN",b,c),EXibin[b][c]&&negHelBin);
			hCorN[b][c]->Divide(hDataN[b][c],hAC[b][c]);
			GraphTop[b][c] = hDataN[b][c]->GetMaximum();					
			hDataP[b][c] = (TH1F*)h->Clone(Form("h%i%iP",b,c));
			hCorP[b][c] = (TH1F*)h->Clone(Form("h%i%iP",b,c));
			hDataP[b][c]->SetFillColor(43);
			hCorP[b][c]->SetFillColor(43);
			tData->Draw(Form("COS_pi_y>>h%i%iP",b,c),EXibin[b][c]&&posHelBin,"SAME");
			hCorP[b][c]->Divide(hDataP[b][c],hAC[b][c]);
			
			
			hCorP[b][c]->Draw();
			hCorN[b][c]->Draw("SAME");
		
			negN[b][c] = hCorN[b][c]->GetEntries();							
			enegN[b][c] = sqrt(hDataN[b][c]->GetEntries());
			posN[b][c] = hCorP[b][c]->GetEntries();	
			eposN[b][c] = sqrt(hDataP[b][c]->GetEntries());
			if(hCorP[b][c]->GetMaximum() > GraphTop[b][c]) {
				GraphTop[b][c] = hCorP[b][c]->GetMaximum();
				hCorN[b][c]->SetMaximum(hCorP[b][c]->GetMaximum());
			}	
			...
			mytex=new TLatex(-0.9, 0.9*GraphTop[b][c],Form("P = %5.3f ",tP));			
