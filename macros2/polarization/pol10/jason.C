/*
this is a macro to written by will to display the text files outputed by Stage4.C
*/
...
void jason(){
    ...
    Plot(nsfStruct,"E_{#gamma} (GeV)","Polarization");
	//Plot(nsfStruct,"cos#theta^{*}_{#Xi^{-}}","Polarization");
...
void Plot(DataStruct & aStruct, TString xTitle, TString yTitle) {
    ...
    for (Int_t i = 0; i < nSeries; i++) {
        ...
        gr[i]->GetYaxis()->SetRangeUser(-2, 2);
