...
void Stage2()
{
	...
	Int_t  YieldDataExErr[NPiBins][NEBins], YieldDataEyErr[NPiBins][NEBins], YieldDataEzErr[NPiBins][NEBins];
	Double_t YieldDataXix[NPiBins][NXiBins], YieldDataXiy[NPiBins][NXiBins], YieldDataXiz[NPiBins][NXiBins];
	Int_t  YieldDataXixErr[NPiBins][NXiBins], YieldDataXiyErr[NPiBins][NXiBins], YieldDataXizErr[NPiBins][NXiBins];
	//MC
	Double_t YieldMCEx[NPiBins][NEBins], YieldMCEy[NPiBins][NEBins], YieldMCEz[NPiBins][NEBins];
	Int_t  YieldMCExErr[NPiBins][NEBins], YieldMCEyErr[NPiBins][NEBins], YieldMCEzErr[NPiBins][NEBins];
	Double_t YieldMCXix[NPiBins][NXiBins], YieldMCXiy[NPiBins][NXiBins], YieldMCXiz[NPiBins][NXiBins];
	Int_t  YieldMCXixErr[NPiBins][NXiBins], YieldMCXiyErr[NPiBins][NXiBins], YieldMCXizErr[NPiBins][NXiBins];
	//Gen	
	Double_t YieldGenEx[NPiBins][NEBins], YieldGenEy[NPiBins][NEBins], YieldGenEz[NPiBins][NEBins];
	Int_t  YieldGenExErr[NPiBins][NEBins], YieldGenEyErr[NPiBins][NEBins], YieldGenEzErr[NPiBins][NEBins];
	Double_t YieldGenXix[NPiBins][NXiBins], YieldGenXiy[NPiBins][NXiBins], YieldGenXiz[NPiBins][NXiBins];
	Int_t  YieldGenXixErr[NPiBins][NXiBins], YieldGenXiyErr[NPiBins][NXiBins], YieldGenXizErr[NPiBins][NXiBins];
	...
	{
		hAngleDataEx[j] = (TH1F*)hAngleX->Clone(Form("hAngleData-EBin%i-x",j));
		hAngleDataEy[j] = (TH1F*)hAngleY->Clone(Form("hAngleData-EBin%i-y",j));
		hAngleDataEz[j] = (TH1F*)hAngleZ->Clone(Form("hAngleData-EBin%i-z",j));
		
		hAngleMCEx[j] = (TH1F*)hAngleX->Clone(Form("hAngleMC-EBin%i-x",j));
		hAngleMCEy[j] = (TH1F*)hAngleY->Clone(Form("hAngleMC-EBin%i-y",j));
		hAngleMCEz[j] = (TH1F*)hAngleZ->Clone(Form("hAngleMC-EBin%i-z",j));
		
		hAngleGenEx[j] = (TH1F*)hAngleX->Clone(Form("hAngleGen-EBin%i-x",j));
		hAngleGenEy[j] = (TH1F*)hAngleY->Clone(Form("hAngleGen-EBin%i-y",j));
		hAngleGenEz[j] = (TH1F*)hAngleZ->Clone(Form("hAngleGen-EBin%i-z",j));
	}
	//loop over Xi bins
	for (int j = 0; j < NXiBins; j++)
	{
		hAngleDataXix[j] = (TH1F*)hAngleX->Clone(Form("hAngleData-XiBin%i-x",j));
		hAngleDataXiy[j] = (TH1F*)hAngleY->Clone(Form("hAngleData-XiBin%i-y",j));
		hAngleDataXiz[j] = (TH1F*)hAngleZ->Clone(Form("hAngleData-XiBin%i-z",j));
		
		hAngleMCXix[j] = (TH1F*)hAngleX->Clone(Form("hAngleMC-XiBin%i-x",j));
		hAngleMCXiy[j] = (TH1F*)hAngleY->Clone(Form("hAngleMC-XiBin%i-y",j));
		hAngleMCXiz[j] = (TH1F*)hAngleZ->Clone(Form("hAngleMC-XiBin%i-z",j));
		
		hAngleGenXix[j] = (TH1F*)hAngleX->Clone(Form("hAngleGen-XiBin%i-x",j));
		hAngleGenXiy[j] = (TH1F*)hAngleY->Clone(Form("hAngleGen-XiBin%i-y",j));
		hAngleGenXiz[j] = (TH1F*)hAngleZ->Clone(Form("hAngleGen-XiBin%i-z",j));
	}
	...
	for (int i = 0; i < NPiBins; i++)
	{
		...
		for (int j = 0; j < NEBins; j++)
		{
			...
			hDataEx[i][j] = (TH1F*)fhS1->Get(Form("hData-PixBin%i-EBin%i",i,j));
			...
			cout << " start of hist filling " <<endl;
		    hAngleDataEx[j]->SetBinContent(i + 1,YieldDataEx[i][j]);
			cout << " mid of hist filling " <<endl;
			hAngleDataEx[j]->SetBinError(i + 1,YieldDataExErr[i][j]);
			cout << " end of hist filling " <<endl;
			cout << "done with X" <<endl;
		  	//*///................................................................................................................................
			
			//*//<........................................................Y.....................................................
			//get the yields
			cDataEy->cd(i + 4*j + 1);
			hDataEy[i][j] = (TH1F*)fhS1->Get(Form("hData-PiyBin%i-EBin%i",i,j));
			if (datacount.compare(fit) == 0)
			{
			...
			//get the yields
			cDataEz->cd(i + 4*j + 1);
			hDataEz[i][j] = (TH1F*)fhS1->Get(Form("hData-PizBin%i-EBin%i",i,j));
			if (datacount.compare(fit) == 0)
			{
			...
			//get the yields
			cMCEx->cd(i + 4*j + 1);
			hMCEx[i][j] = (TH1F*)fhS1->Get(Form("hMC-PixBin%i-EBin%i",i,j));
			if (simcount.compare(fit) == 0)
			{
			...
			//get the yields
			cMCEy->cd(i + 4*j + 1);
			hMCEy[i][j] = (TH1F*)fhS1->Get(Form("hMC-PiyBin%i-EBin%i",i,j));
			if (simcount.compare(fit) == 0)
			{
			...
			//get the yields
			cMCEz->cd(i + 4*j + 1);
			hMCEz[i][j] = (TH1F*)fhS1->Get(Form("hMC-PizBin%i-EBin%i",i,j));
			if (simcount.compare(fit) == 0)
			{
				if((i == 1 && j == 2)||(i == 2 && j == 0 )){Double_t fitWidth = 0.0049;fitWidthFloat = 0.0008;}
			...
			//get the yields
			cGenEx->cd(i + 4*j + 1);
			hGenEx[i][j] = (TH1F*)fhS1->Get(Form("hGen-PixBin%i-EBin%i",i,j));
			...
			//get the yields
			cGenEy->cd(i + 4*j + 1);
			hGenEy[i][j] = (TH1F*)fhS1->Get(Form("hGen-PiyBin%i-EBin%i",i,j));
			//count rather than fit. generated events should never need to be fitted.
			...
			//get the yields
			cGenEz->cd(i + 4*j + 1);
			hGenEz[i][j] = (TH1F*)fhS1->Get(Form("hGen-PizBin%i-EBin%i",i,j));
			//count rather than fit. generated events should never need to be fitted.
	...
	for (int i = 0; i < NPiBins; i++)
	{
		...
		for (int j = 0; j < NXiBins; j++)
		{
			...
			//get the yields
			cDataXix->cd(i + 4*j + 1);
			hDataXix[i][j] = (TH1F*)fhS1->Get(Form("hData-PixBin%i-XiBin%i",i,j));
			...
			//get the yields
			cDataXiy->cd(i + 4*j + 1);
			hDataXiy[i][j] = (TH1F*)fhS1->Get(Form("hData-PiyBin%i-XiBin%i",i,j));
			...
			//get the yields
			cDataXiz->cd(i + 4*j + 1);
			hDataXiz[i][j] = (TH1F*)fhS1->Get(Form("hData-PizBin%i-XiBin%i",i,j));
			...
			//get the yields
			cMCXix->cd(i + 4*j + 1);
			hMCXix[i][j] = (TH1F*)fhS1->Get(Form("hMC-PixBin%i-XiBin%i",i,j));
			if (simcount.compare(fit) == 0)
			{
			...
			//get the yields
			cMCXiy->cd(i + 4*j + 1);
			hMCXiy[i][j] = (TH1F*)fhS1->Get(Form("hMC-PiyBin%i-XiBin%i",i,j));
			if (simcount.compare(fit) == 0)
			{
			...
			//get the yields
			cMCXiz->cd(i + 4*j + 1);
			hMCXiz[i][j] = (TH1F*)fhS1->Get(Form("hMC-PizBin%i-XiBin%i",i,j));
			if (simcount.compare(fit) == 0)
			{
				if((i == 1 && j == 2)||(i == 2 && j == 0 )){Double_t fitWidth = 0.0049;fitWidthFloat = 0.0008;}
			...
			//get the yields
			cGenXix->cd(i + 4*j + 1);
			hGenXix[i][j] = (TH1F*)fhS1->Get(Form("hGen-PixBin%i-XiBin%i",i,j));
			//count rather than fit. generated events should never need to be fitted. 
			...
			//get the yields
			cGenXiy->cd(i + 4*j + 1);
			hGenXiy[i][j] = (TH1F*)fhS1->Get(Form("hGen-PiyBin%i-XiBin%i",i,j));
			//count rather than fit. generated events should never need to be fitted.
			...
			//get the yields
			cGenXiz->cd(i + 4*j + 1);
			hGenXiz[i][j] = (TH1F*)fhS1->Get(Form("hGen-PizBin%i-XiBin%i",i,j));
			//count rather than fit. generated events should never need to be fitted.
