...
void XiFile(TString passed)
{
	...
	for (Long64_t i=0; i<nentries; i++)
	{
		...
		Bool_t cut = sqrt((mmkpkp-MASS_Xi)*(mmkpkp-MASS_Xi)/(widthA*widthA)+(mmkpkppim-MASS_Lambda)*(mmkpkppim-MASS_Lambda)/(widthB*widthB)) < 1;
		//Bool_t cut = ((mmkpkp-MASS_Xi)/(widthA)) < 1;
		if ( cut )
		{
