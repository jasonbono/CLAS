...
void Fiducial()
{
	...
	//you may parse this data however you like.

	// Use an example of typical values:
	float myParticleSector = 3;  //call FidFunc() with this value
	float myParticleMomentum = 0.5; 
	float myParticleTheta = 30;  
	Float_t myInput[2] = {myParticleMomentum,myParticleTheta}; //call FidFunc() with this value
	float myParticlePhi = 12;   //compare with the output of FidFunc() 



	//say the track is negitive...
	float myLowPhi = FidFunc(myInput,parL_Neg[myParticleSector - 1]); 
	float myHighPhi = FidFunc(myInput,parH_Neg[myParticleSector - 1]);

	/*if track was taken to be positive one would have done...
	float myLowPhi = FidFunc(myInput,parL_Pos[myParticleSector - 1]); 
	float myHighPhi = FidFunc(myInput,parH_Pos[myParticleSector - 1]);
	*/



	//can filter an event based on a single track by
