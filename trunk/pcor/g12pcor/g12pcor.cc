...
void Init_g12pcor(double __p_vals[2][6][180][3],int __num_pars_p[2][6][180],
...
void g12TaggerCor_(float *__eg,int *__eid,int *__runNumber,
           float *__egCor){
  *__egCor = g12TaggerCor(*__eg,*__eid,*__runNumber);
}

void g12MomCor_(float __p3[3],int *__q,float __p3cor[3]){
  g12MomCor(__p3,*__q,__p3cor);
}

void g12FiducialCut_(float __p3[3],int *__q,int *__cut){
  bool is_good = g12FiducialCut(__p3,*__q);
  if(is_good) *__cut = 1;
  else *__cut = 0;
}
//_____________________________________________________________________________

/// Tagger correction for the g12 run period
 ...
 *  find this file, the user's CLAS_PARMS enviornment variable must be set).
...
float g12TaggerCor(float __eg,int __eid,int __runNumber){
  static const int nruns = 961;
  static int first_g12_run = 56363;
  static bool first_call = true;
  static float eid_erg[767];
  static float offset[nruns];

  if(first_call){ // only do this on the 1st call to the function
    // read in the tagger (gamma) energy parameters
    char file[200];
    sprintf(file,"%s/pcor/g12/tagger_energies.dat",std::getenv("CLAS_PARMS"));
    std::ifstream inFile_eg(file);
    // check to make sure the file opened properly
    if(!inFile_eg.is_open()){
      printf("Error!!!! <g12TaggerCor> Can NOT open tagger_energies.dat. This ");
      printf("file should be located in CLAS_PARMS/pcor/g12. Check ");
      printf("to make sure your CLAS_PARMS enviornment variable is properly ");
      printf("set.\n");
    ...
    float erg_corrected;
    int nhits;
    float erg_uncorrected;
    int count = 0;
    while(inFile_eg >> eid >> erg_corrected >> nhits >> erg_uncorrected){
      eid_erg[eid-1] = erg_corrected;
      count++;
    }
    inFile_eg.close(); // close the input file
    if(count != 767){
      printf("Error!!!! <g12TaggerCor> Read incorrect number of E-paddles ");
      printf("[%d instead of 767]. \n",count);
      return -666.;
    }
    // read in beam offsets
    sprintf(file,"%s/pcor/g12/beam_offset.dat",std::getenv("CLAS_PARMS"));
    std::ifstream inFile_offset(file);
    int run;
    float beam_off;
    for(int i = 0; i < nruns; i++) offset[i] = 0.;

    while(inFile_offset >> run){
      inFile_offset >> beam_off;
      offset[run - first_g12_run] = beam_off;
    }
  ...
  if(__eid < 1 || __eid > 767){
    printf("Warning <g12TaggerCor> E-paddle out of range [id = %d].",__eid);
    printf(" No correction will be applied. \n");
    return __eg;
  }

  float Eg = eid_erg[__eid-1]; // tagger energy
  Eg += offset[__runNumber - first_g12_run]; // add beam correction if we have one

  return Eg; // return the corrected energy
}
//_____________________________________________________________________________

/// Momentum corrections for g12.
...
void g12MomCor(float __p3[3],int __q,float __p3cor[3]){
  ...
  if(firstCall){ // on first call, do the following...
    ...
    Init_g12pcor(p_vals,num_pars_p,phi_vals,num_pars_phi,lambda_vals,
         num_pars_lambda);
    firstCall = false;
  }
...
void ReadBinsFile(double __phiBins_min[12],double __phiBins_max[12],
  ...
  char file[200];
  sprintf(file,"%s/pcor/g12/binning.dat",std::getenv("CLAS_PARMS"));
  std::ifstream binsFile(file);
  int num_bins;

  if(!binsFile.is_open()){ // couldn't open the file
    printf("Error!!! <ReadBinsFile> Could NOT open binning.dat. ");
    printf("This function is called by g12MomCor and should be located in ");
    printf("CLAS_PARMS/pcor/g12, make sure your CLAS_PARMS ");
...
void Init_g12pcor(double __p_vals[2][6][180][3],int __num_pars_p[2][6][180],
  ...
  double x[10];
  std::string file_base = std::getenv("CLAS_PARMS");
  file_base += "/pcor/g12/pars_";
  char fileName[200];
  ...
  for(int s = 1; s <= 6; s++){ // loop over sectors
    ...
    printf("Error!!!! <Init_g12pcor> Could NOT open %s. ",fileName);
    printf("This function is called by g12MomCor to read in the ");
