...
parseListFN (char *FList, bosioFNList *flist)
{
  ...
  do
  {
    ...
    if (flist->nFN == MAXFNLIST) {
      fprintf(stderr,"parseListFN : Error : Max number of files in list %i is exeeded => stop parsing\n",MAXFNLIST);
      FREE(f);
      return(-1);
    }
    *end=0;
...
bosOPEN(char *file, char *flag, int *descriptor)
{
  ...
  if(!reopen)    /* Process New File      */
  {
    ...
    {
      int i;
      for(i=0; i<Blun->flist.nFN; i++) fprintf(stderr,"file[%2d]->%s<-\n",i,Blun->flist.FN[i]);
    }
  }
  else
  {
    DPR("bosOPEN : close file, but remember lun\n");
    fprintf(stderr,"bosopen.c: close file, but remember lun\n");
    lun = Blun->unit-1;
    Blun->reopen = 1;
    bosClose(*descriptor);
    fprintf(stderr,"bosopen.c: old file closed\n");
  }
  ...
  */
  ...
  switch (*flag)
  {
    ...
    case 'U': infu[lun]->action = 4;
              ...
              action = "r+b";
              break;
    default:  fprintf(stderr,"bosopen: wrong parameters\n");
              FREE(infu[lun]);
              infu[lun] = 0;
              error = EBIO_WRONGPARAM;
              goto exit;
  ...
  else if(reopen && infu[lun]->action == 1)
  {
    ...
    {
      fprintf(stderr,"No more continuation files, infu[%d]=%08x\n",lun,infu[lun]);
      error = EBIO_EOF;
      goto exit;
    }
  ...
  if(infu[lun]->action == 2 && Blun->splitmb != 0)
  {
    ...
    str[i] = '\0';
    fprintf(stderr,"checking how much disk space left in directory %s\n",str);
    if(status=statvfs(str,&buf))
    {
      fprintf(stderr,"bosopen.c: ERROR=%d calling statvfs(), so can not check how much disk space left ...\n",status);
    }
    else
    {
      fprintf(stderr,"bosopen.c: total %d blocks, free %d blocks, available %d blocks (1 block = %d bytes)\n",
              buf.f_blocks,buf.f_bfree,buf.f_bavail,buf.f_frsize);
      avail = buf.f_bavail*(buf.f_frsize/1024);
      fprintf(stderr,"bosopen.c: so we have %d kbytes available space\n",avail);
      if(avail > Blun->splitmb*1024)
      {
        fprintf(stderr,"bosopen.c: we can continue to write file.\n");
      }
      else
      {
        fprintf(stderr,"bosopen.c: ERROR: there is no enough space for writing.\n");
        fprintf(stderr,"bosopen.c: ERROR: disk could be overflow in any moment !!!\n");
        fprintf(stderr,"bosopen.c: ERROR: !!!!! STOP WRITING IMMIDEATELY !!!\n");
      }
    }
  }
#endif
  ...
  else              /* local file */
  {
    ...
    {
      fprintf(stderr,"bosopen: error in copen = %d\n",error); fflush(stdout);
      /* error = -7; */
      goto exit;
    }
  }
  ...
  if(infu[lun]->action == 2 && Blun->splitmb != 0)
  {
    ...
    {
      char str[100], *strval = "                    ";
      fprintf(stderr,"bosopen.c: check for partition swaping: file->%s<-\n",Blun->finame);
      strcpy(str,"checkdisk ");
      sprintf(stderr,strval," %4d",Blun->splitmb * 2);
      strcat(str,strval);
      fprintf(stderr,"bosopen.c: system call: >%s<\n",str);
      system(str);
      fprintf(stderr,"bosopen.c: system call completed\n");
    }
  }
...
/* release slot */
  ...
  {
    fprintf(stderr,"bosClose : Not releasing slot\n");
  }
