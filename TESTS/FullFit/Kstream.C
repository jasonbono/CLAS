...
void Kstream::_MainFitter(){
  ...
  double theta[numParts]; //added for neutrals
  ...
  for(i = 0; i < numParts; i++){
	...
	if(_p_names[i] == "gamma") mass[i] = 0.0000000000000000;
	}    
   else {
	initialy(3*i+1) = _p4in[i].P(); // |p|
	initialy(3*i+2) = LambdaTrack(_p4in[i]); // tracking angle lambda
	initialy(3*i+3) = PhiTrack(_p4in[i]); // tracking angle phi
  ...
  for(int iter = 0; iter < 10; iter++){
	...
	pz_inv += p3_miss.Z();	
      ...
      c(4) = e_inv*e_inv - px_inv*px_inv - py_inv*py_inv - pz_inv*pz_inv - 				     pow(_invariantMass,2);  
    }
    if(_misswitch){
      for(i = 0; i < 3; i++){
	a(0,i) = -x(i)/e_miss; 
	...
	a(4,0) = 2.*(e_inv*(x(0)/e_miss) - px_inv);
	a(4,1) = 2.*(e_inv*(x(1)/e_miss) - py_inv);
	a(4,2) = 2.*(e_inv*(x(2)/e_miss) - pz_inv);
      }
    }
    aT.Transpose(a);
    b(0,0) = 1.;
    ...
    if(_p_names[i] == "n" or _p_names[i] == "gamma") {
	b(0,3*i+1) = -p[i]/erg[i];
	b(1,3*i+1) = sin(theta[i])*cos(phi[i]);
	b(2,3*i+1) = sin(theta[i])*sin(phi[i]);
	b(3,3*i+1) = cos(theta[i]);
	b(0,3*i+2) = 0.;
	b(1,3*i+2) = p[i]*cos(theta[i])*cos(phi[i]);
	b(2,3*i+2) = p[i]*cos(theta[i])*sin(phi[i]);
	b(3,3*i+2) = -p[i]*sin(theta[i]);
	b(0,3*i+3) = 0.;
	b(1,3*i+3) = -p[i]*sin(theta[i])*sin(phi[i]);
	b(2,3*i+3) = p[i]*sin(theta[i])*cos(phi[i]);
	b(3,3*i+3) = 0.;
	}    
   else {    
      b(0,3*i+1) = -p[i]/erg[i];
    ...
    gb = b * _V * bT;  
    gb.Determinant(d1,d2);      
    gb.Invert();
    gb.Determinant(d1,d2);    
    if(_misswitch){
      xsi = ((aT * gb * a).Invert())*(aT * gb * c); 
      x -= xsi;
      delta = _V * bT * gb * (c - a*xsi);
    }
    else delta = _V * bT * gb * c;
    y -= delta;
    eps = initialy - y;
    TMatrixDColumn(epsmatrixT,0) = eps;
    epsmatrix.Transpose(epsmatrixT);
    _chi2 = (epsmatrix * bT * gb * b * epsmatrixT)(0,0); 
    ...
    TMatrixDColumn(epsmatrixT,0) = eps;
  if(_misswitch){
    V_fit = _V - _V*bT*gb*b*_V 
      + _V*bT*gb*a*((aT*gb*a).Invert())*aT*gb*b*_V;
  }
  else 
    V_fit = _V - _V*bT*gb*b*_V;
  _pulls.resize(dim);
  ...
  if(_misswitch)
    ...
    this->_SetMissingParticleV((aT * gb * a).Invert(),x);    
}
...
void Kstream::_SetMissingParticleV(const TMatrixD &__missingV,
				       const TVectorD &__x){
  _Vmiss.ResizeTo(3,3);
  ...
  _Vmiss(0,0) = sigma_px_2*(dp_dpx*dp_dpx) 
			   + sigma_py_2*(dp_dpy*dp_dpy)
			   + sigma_pz_2*(dp_dpz*dp_dpz);
			   
  _Vmiss(1,1) = sigma_px_2*(dlam_dpx*dlam_dpx) 
			   + sigma_py_2*(dlam_dpy*dlam_dpy)
			   + sigma_pz_2*(dlam_dpz*dlam_dpz);
  
  _Vmiss(2,2) = sigma_px_2*(dphi_dpx*dphi_dpx) 
			   + sigma_py_2*(dphi_dpy*dphi_dpy)
			   + sigma_pz_2*(dphi_dpz*dphi_dpz);			   
			     
  _Vmiss(1,0) = sigma_px_2*(dp_dpx*dlam_dpx) 
			   + sigma_py_2*(dp_dpy*dlam_dpy)
			   + sigma_pz_2*(dp_dpz*dlam_dpz);
			   
  _Vmiss(0,1) = _Vmiss(1,0);
    
  _Vmiss(2,0) = sigma_px_2*(dp_dpx*dphi_dpx) 
			   + sigma_py_2*(dp_dpy*dphi_dpy)
			   + sigma_pz_2*(dp_dpz*dphi_dpz);
			   			   
  _Vmiss(0,2) = _Vmiss(2,0);  
    
  _Vmiss(2,1) = sigma_px_2*(dlam_dpx*dphi_dpx) 
			   + sigma_py_2*(dlam_dpy*dphi_dpy)
			   + sigma_pz_2*(dlam_dpz*dphi_dpz);
			     			     
  _Vmiss(1,2) = _Vmiss(2,1); 
}				       
