...
class Job:
    ...
    def __init__(self, run, maxindex, index='A00'):
        ...
        outdir = '/mss/clas/g12/production/pass1/user_skims/jbono/kpkp'
        ...
        self.output_files = [(
            'out.bos',
            path.join(outdir, '.'.join(['kpkp',run,index[:-1]+'X','bos']))
            )]
    ...
    def run(self, timelimit=4.):
        ...
        cmd = 'kpkp-skim -oout.bos input.*.bos'
        if not run(cmd, outfile='out.bos', timeout=(fin_time - time())):
            return False

        if not check_output(self):
            return False
