...
march 29 2012: jb v01:  in this version i attemped to add the function IsBeamLargerThanTwo, IsTimingOk and implement this 




march 29 2012: jb v00  this is the original skim template that i used back in October 2011 to do my orignal skim.
it reques more than 1 kaon and more than zero pions
  ...
  for (i = 1; i < argc; ++i) {
	...
	while ((max ? Nevents < max : 1) && ret) {
	  ...
	  if (ret == DISIO_DATA) {
		...
		int TimeReq = IsTimingOk(event,verbose,Silent); //jb v01
		//int BeamReq = IsBeamGreaterThanTwo(event,verbose,Silent);  //jb v01
...
{	
    int ret;
    //cout<<"test"<<endl;
    cout<<evt.N(KPlus)<<" "<<evt.N(PiMinus)<<endl;
	if((ret = (evt.N(KPlus)>1)&& (evt.N(PiMinus)>0))){
        cout<<"PASS!"<<endl;
	}
	return ret;
}*/
...
int ProcessEvent(clasEvent &evt,int verbose,int silent){	
	return ((evt.N(KPlus)>1)&&(evt.N(PiMinus)>0));
}


//jb v01
int IsTimingOk(clasEvent &evt, int verbose, int silent)
{
	float scv=my_event.scVtime();
	float stv=my_event.stVtime();
	return ( (abs(stv - scv) < 3.0 ) )	
}



//jb v01
//int IsBeamGreaterThanTwo(clasEvent &evt, int verbose, int silent)
//{
//	return (evt.beam>2)
